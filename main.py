...
async def resettotal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("♻️ Đã reset bảng tổng!")
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for num in nums:
        ...
        if len(user["history"]) < 2:
            msgs.append(f"📥 Nhận: {num} → chờ thêm...")
            continue
        ...
        if percent >= 30:
            ...
            await update.message.reply_text("🛑 +30% → RESET NGAY")
            return
