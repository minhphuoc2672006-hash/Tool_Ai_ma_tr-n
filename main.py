...
ROBOT_ANALYZE_DELAY = float(os.getenv("ROBOT_ANALYZE_DELAY", "1.5"))
    ...
    try:
        ...
        for i in range(24):
            ...
            for ex in (eye1_x, eye2_x):
                ...
                draw.rounded_rectangle((ex - eye_w // 2, top, ex + eye_w // 2, top + eye_h),
                                       radius=12, fill=(52, 225, 255, 220))

            arm_y = int(h * 0.71)
            swing = int(12 * math.sin(2 * math.pi * phase + math.pi / 2))
            draw.line((int(w * 0.34), arm_y, int(w * 0.30) + swing, arm_y + 50),
                      fill=(110, 210, 255, 90), width=10)
            draw.line((int(w * 0.62), arm_y, int(w * 0.66) - swing, arm_y + 50),
                      fill=(110, 210, 255, 90), width=10)
            ...
            draw.ellipse((int(w * 0.43) - glow_w, ring_y - glow_h, int(w * 0.43) + glow_w, ring_y + glow_h),
                          fill=(65, 220, 255, alpha))
...
def detect_explicit_pair_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    if a == b and c == d and a != c:
        out.append({"name": "2-2", "detail": "Mẫu chia cặp 2-2", "score": 88, "hint": a})

    if len(labels) >= 8:
        t = labels[-8:]
        # Cầu 2-2 mở rộng: AABB AABB hoặc BBAA BBAA
        if t[0] == t[1] and t[2] == t[3] and t[4] == t[5] and t[6] == t[7]:
            p1, p2, p3, p4 = t[0], t[2], t[4], t[6]
            if p1 == p3 and p2 == p4 and p1 != p2:
                out.append({
                    "name": "CẦU 2-2",
                    "detail": "Nhịp 2-2 lặp đều trong 8 mẫu gần nhất",
                    "score": 95,
                    "hint": p1,
                })

        if t[0] == t[2] == t[4] and t[1] == t[3] == t[5] and t[0] == t[6] and t[1] == t[7] and t[0] != t[1]:
            out.append({
                "name": "XEN KẼ SÂU",
                "detail": "Luân phiên đều trong 8 mẫu gần nhất",
                "score": 96,
                "hint": t[0],
            })
...
def detect_transition_phase(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if diff >= 0.45:
        ...
        return {
            "name": "CHUYỂN PHA",
            "detail": f"Đoạn gần nhất nghiêng mạnh về {winner} và lệch rõ so với đoạn trước",
            "score": 88,
            "hint": winner,
        }
    ...
    if diff >= 0.30 and abs(recent_streak - prev_streak) >= 2:
        ...
        return {
            "name": "CHUYỂN PHA",
            "detail": f"Nhịp gần đây đổi pha sang {winner}",
            "score": 82,
            "hint": winner,
        }
...
def detect_complex_cycle(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if len(runs) >= 4:
        ...
        if r[0] == r[2] and r[1] == r[3] and r[0] != r[1] and lbls[0] != lbls[1] and lbls[2] != lbls[3]:
            return {
                "name": "CẦU PHỨC HỢP",
                "detail": f"Run-length {r[0]}-{r[1]} lặp theo cặp",
                "score": min(92, 86 + r[0] + r[1]),
                "hint": tail[-1],
            }
    ...
    if len(runs) >= 6:
        ...
        if r[0] == r[3] and r[1] == r[4] and r[2] == r[5] and len({r[0], r[1], r[2]}) >= 2:
            return {
                "name": "CHU KỲ MỞ RỘNG",
                "detail": f"Nhịp {r[0]}-{r[1]}-{r[2]} lặp",
                "score": min(94, 84 + sum(r[-3:])),
                "hint": tail[-1],
            }
...
def detect_nested_zigzag(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if changes_ratio >= 0.72 and ("TT" in s or "XX" in s):
        return {
            "name": "XEN KẼ BIẾN THỂ",
            "detail": f"Chuỗi đổi liên tục nhưng có nhịp gãy nhẹ: {s}",
            "score": 84,
            "hint": opposite_label(tail[-1]),
        }
    return None
...
def detect_false_trap(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    for side in (HIGH_LABEL, LOW_LABEL):
        ...
        if tail[-6:-3] == [side, side, side] and tail[-3] == other and tail[-2:] == [side, side]:
            return {
                "name": "CẦU BẪY",
                "detail": f"Cụm {side} x3 bị chọc 1 nhịp rồi hồi lại",
                "score": 89,
                "hint": None,
            }
    return None
...
def extract_chart_features(labels: List[str]) -> Dict[str, Any]:
    if not labels:
        return {
            "trend": 0.0, "trend_label": "TRUNG TÍNH", "reversal_rate": 0.0, "smoothness": 0.0,
...
def update_confirm_state(state: Dict[str, Any], report: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    # Không chặn phân tích nữa; chỉ ghi nhận nhịp đang được bám theo.
    if score >= 92:
        reason = f"Cầu rất rõ: {top.get('name','-')} ({score}%)"
    elif score >= 82:
        reason = f"Cầu rõ: {top.get('name','-')} ({score}%)"
    else:
        reason = f"Cầu đang chuyển nhịp: {top.get('name','-')} ({score}%)"
...
def prediction_gate(labels: List[str], report: Dict[str, Any], state: Optional[Dict[str, Any]] = None) -> Tuple[bool, str]:
    ...
    if state is not None:
        _, wait_reason = update_confirm_state(state, report)
        state["last_gate_reason"] = wait_reason

    # Bỏ cơ chế chờ xác nhận cầu mới: có tín hiệu là tiếp tục phân tích.
    if total < 6:
        return False, "Chưa đủ dữ liệu tối thiểu"

    if not patterns:
        return True, "Chưa có cầu rõ, ưu tiên tiếp tục theo biểu đồ"

    if top_score < 70 and total < 10:
        return True, f"Cầu còn non: {top_name} ({top_score}%), vẫn phân tích theo biểu đồ"

    if top_score < CLEAR_PATTERN_MIN_SCORE:
        return True, f"Cầu chưa thật rõ: {top_name} ({top_score}%), vẫn bám biểu đồ"
...
def meta_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    last_5_high = float(adv.get("last_5_high", 0.5))
    prev_5_high = float(adv.get("prev_5_high", 0.5))
    chart_pred = predictions.get("chart", {})
    chart_label = chart_pred.get("label")
    chart_conf = float(chart_pred.get("confidence", 50))
    structure_pred = predictions.get("structure", {})
    structure_label = structure_pred.get("label")
    structure_conf = float(structure_pred.get("confidence", 50))
    ...
    for name, pred in predictions.items():
        ...
        # Ưu tiên biểu đồ cầu và xí ngầu hơn thống kê nền.
        if name == "chart":
            score *= 1.22
            if abs(trend) >= 0.18:
                score *= 1.16
        ...
        # Nếu biểu đồ và cấu trúc cùng hướng thì đẩy mạnh thêm.
        if name == "chart" and structure_label == chart_label and structure_conf >= 62:
            score *= 1.08
    ...
    confidence = int(46 + top_ratio * 40 + (agreement - 1) * 4 + (strongest_conf - 50) * 0.18)
    if chart_label == best_label and chart_conf >= 70:
        confidence += 6
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str], dice_rolls_full: Optional[List[List[int]]] = None) -> Dict[str, Any]:
    ...
    # Không chặn nữa vì thiếu đồng thuận; chỉ giảm độ tin cậy để bot vẫn chạy liên tục.
    if meta["top_ratio"] < MIN_CONSENSUS_RATIO or meta["agreement"] < 2 or meta["confidence"] < MIN_FINAL_CONFIDENCE:
        state["last_note"] = f"Biểu đồ dẫn hướng, đồng thuận thấp ({meta['agreement']}/{len(predictions)})"
    else:
...
def build_stats_message(report: Dict[str, Any], state: Dict[str, Any], adv: Dict[str, Any], dice_summary: Optional[Dict[str, Any]] = None) -> str:
    total = report["total"]
    ...
    dice_line = "-"
    if dice_summary and int(dice_summary.get("total_rolls", 0)) > 0:
        av = dice_summary.get("avg_faces", [0.0, 0.0, 0.0])
        st = dice_summary.get("std_faces", [0.0, 0.0, 0.0])
        delta = dice_summary.get("recent_delta", [0.0, 0.0, 0.0])
        dice_line = (
            f"Rolls:{dice_summary.get('total_rolls', 0)} | Avg:{dice_summary.get('avg_total', 0.0):.2f} | "
            f"D1/D2/D3:{av[0]:.2f}/{av[1]:.2f}/{av[2]:.2f} | "
            f"Std:{st[0]:.2f}/{st[1]:.2f}/{st[2]:.2f} | "
            f"Δ:{delta[0]:+.0f}/{delta[1]:+.0f}/{delta[2]:+.0f}"
        )
    return (
        "╔════════════════════════════╗\n"
        "║      ✅ BẢNG THỐNG KÊ      ║\n"
        ...
        f"║ CycleSig : {report.get('cycle_signature', '-')}\n"
        f"║ Repeat   : {report.get('repeat_signature', '-')} x{report.get('repeat_count', 0)}\n"
        f"║ Loạn/Stab: {report.get('chaos_score', 0)}/{report.get('stability_score', 50)}\n"
        f"║ Nhận diện: {report.get('structure_hint', '-')}\n"
        f"║ Ghi chú  : {report.get('repeat_note', '-')}\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"/aimodel - bật/tắt hiển thị AI\n"
        f"/hashmodel - bật/tắt model hash/MD5\n"
        ...
        f"Luồng hoạt động: cập nhật thống kê → biểu đồ cầu → biểu đồ xí ngầu → robot phân tích → chốt cuối.\n"
        f"Xí ngầu được đọc theo từng viên riêng, có độ lệch, biến động ngắn và xu hướng gần nhất."
    )
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(build_stats_message(report_full, state, adv_full, dice_summary))
