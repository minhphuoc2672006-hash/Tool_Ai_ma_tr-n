...
def update_timing_feedback(d: Dict[str, Any], actual_label: str) -> None:
    """Học từ kết quả vừa mới vào để giảm bẻ sớm / bẻ trễ ở lần sau."""
    ...
    last_hint = d.get("last_decision_hint", "TRUNG_LẬP")
    ...
    if pred == actual_label:
        d["timing_hits"] = int(d.get("timing_hits", 0)) + 1
        d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.92
        d["timing_last_state"] = "OK"
        return

    d["timing_misses"] = int(d.get("timing_misses", 0)) + 1

    # Bẻ quá sớm: đã flip nhưng thực tế còn giữ nguyên nhịp cũ.
    if last_hint == "BẺ" and prev_actual in (LOW_LABEL, HIGH_LABEL) and actual_label == prev_actual:
        d["timing_early_miss"] = int(d.get("timing_early_miss", 0)) + 1
        d["timing_bias"] = float(d.get("timing_bias", 0.0)) - (0.10 * weight)
        d["timing_last_state"] = "EARLY"
        return

    # Bẻ quá trễ: đã giữ theo đuôi cũ nhưng thực tế đã đổi nhịp.
    if last_hint in ("THEO", "TRUNG_LẬP") and prev_actual in (LOW_LABEL, HIGH_LABEL):
        flipped = opposite_label(prev_actual)
        if actual_label == flipped:
            d["timing_late_miss"] = int(d.get("timing_late_miss", 0)) + 1
            d["timing_bias"] = float(d.get("timing_bias", 0.0)) + (0.10 * weight)
            d["timing_last_state"] = "LATE"
            return

    # Mất tín hiệu chung: kéo bias về trung tính.
    d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.96
    d["timing_last_state"] = "MISS"
