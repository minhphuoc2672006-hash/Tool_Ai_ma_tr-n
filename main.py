...
WS_URL = "wss://your-websocket-url"  # URL WebSocket game push
if not TOKEN:
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(nums) != 3:
        ...
        return
    await process_dice(user, nums, update)

# ===== PROCESS DICE =====
async def process_dice(user, dice, update: Update):
    total = sum(dice)
    ...
    async with websockets.connect(WS_URL) as ws:
        ...
        while True:
            try:
                msg = await ws.recv()
                data = json.loads(msg)
                dice = data.get("dice")  # Ví dụ: [1,3,5]
                if not dice:
                    continue
                # gửi kết quả cho tất cả user
                for uid, user in users.items():
                    class DummyUpdate:
                        message = type("msg", (), {"reply_text": lambda *a, **kw: None})()
                    await process_dice(user, dice, DummyUpdate())
