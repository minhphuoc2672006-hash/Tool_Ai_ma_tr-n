...
OCR_KEY = os.getenv("OCR_KEY")  # Ẩn key OCR
if not TOKEN:
    raise Exception("❌ Thiếu TOKEN")
if not OCR_KEY:
    raise Exception("❌ Thiếu OCR_KEY")
...
# ===== OCR CLOUD =====
async def ocr_api(path):
    url = "https://api.ocr.space/parse/image"
    headers = {"apikey": OCR_KEY}
    files = {"file": open(path, "rb")}
    data = {"OCREngine": 2, "language": "eng"}
    ...
    try:
        ...
        for line in text.splitlines():
            ...
            numbers.extend([n for n in nums if 3 <= n <= 18])
    except:
        pass
    return numbers

# ===== OCR ZIGZAG LOCAL (Backup) =====
def read_grid_zigzag(path):
    ...
    for cnt in contours:
        ...
        if 25 < w < 120 and 25 < h < 120:
            ...
            txt = pytesseract.image_to_string(roi, config="--psm 6 digits").strip()
            if txt.isdigit():
                n = int(txt)
                if 3 <= n <= 18:
                    boxes.append((x,y,n))
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        # Thử OCR cloud trước
        results = await ocr_api(path)
        if not results:
            # fallback OCR local zigzag
