...
ANALYSIS_DELAY_SECONDS = float(os.getenv("ANALYSIS_DELAY_SECONDS", "5"))
...
def choose_balanced_label(chat_id: int, total: int, low: int, high: int, last_label: Optional[str], streak: int) -> str:
...
def forecast_next(chat_id: int, report: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    if abs(probs[LOW_LABEL] - probs[HIGH_LABEL]) >= 0.02:
        ...
        best_label = choose_balanced_label(chat_id, total, low, high, last_label, streak)
    ...
    if delta < 0.08:
        ...
        status = "TRENDING"
        message = f"Xu hướng nghiêng về {best_label}."

    if streak >= 6:
        status = "STREAK"
        message = f"Chuỗi rất mạnh: {last_label} x{streak}."

    if cycle_score >= 70 and cycle_len:
        status = "CYCLE"
        message = f"Phát hiện vòng lặp (size {cycle_len})."
...
def meta_ai_judge(chat_id: int, report: Dict[str, Any], ai: Dict[str, Any]) -> Dict[str, Any]:
    ...
    long_mode = report.get("long_mode", "BALANCED")
    long_score = int(report.get("long_score", 0))
    low = int(report.get("low", 0))
    high = int(report.get("high", 0))
    total = int(report.get("total", 0))
    gap = abs(low - high)

    low_score = probs[LOW_LABEL] * 100.0
    high_score = probs[HIGH_LABEL] * 100.0

    if long_mode.startswith("IMBALANCED") and long_score >= 2:
        if low > high:
            low_score += 8
        else:
            high_score += 8
        confidence += 6
        reasons.append("lệch tổng thể")

    if alt and alt_ratio >= 0.90 and last_label in (LOW_LABEL, HIGH_LABEL):
        if last_label == LOW_LABEL:
            high_score += 7
        else:
            low_score += 7
        confidence += 5
    ...
    if streak >= 6 and last_label in (LOW_LABEL, HIGH_LABEL):
        if last_label == LOW_LABEL:
            ...
            low_score += 10
        confidence += 8
        reasons.append(f"chuỗi dài {last_label} x{streak}")

    if cycle_score >= 70 and cycle_len:
        confidence += 4
        reasons.append(f"vòng lặp size {cycle_len}")

    if mode == "NOISY" or (vol >= 0.85 and ent >= 0.85):
        confidence -= 10
        reasons.append("nhiễu cao")

    if gap <= 2 and mode in ("BALANCED", "NORMAL"):
        confidence -= 4
        reasons.append("cân bằng mạnh")

    if mode == "WARMUP":
        confidence = min(confidence, 45)
        reasons.append("chưa đủ dữ liệu")

    if abs(low_score - high_score) < 1.0:
        fallback = choose_balanced_label(chat_id, total, low, high, last_label, streak)
        if fallback == LOW_LABEL:
            low_score += 2.0
        else:
            high_score += 2.0

    final_probs = normalize_probs({
        LOW_LABEL: low_score,
        HIGH_LABEL: high_score,
    })

    final_label = max(final_probs, key=final_probs.get)
    final_confidence = int(round(max(final_probs.values()) * 100))
...
# UI
# =========================
def build_stats_message(d: Dict[str, Any]) -> str:
    total = d["low_count"] + d["high_count"]
    return (
        "📊 THỐNG KÊ\n"
        f"• {LOW_LABEL}: {d['low_count']} | {HIGH_LABEL}: {d['high_count']}\n"
        f"• Tổng: {total} | Mode: {d.get('mode', 'WARMUP')} | Lượt: {d.get('updates', 0)}"
    )


def build_analysis_message(report: Dict[str, Any], ai: Dict[str, Any], meta: Dict[str, Any], d: Dict[str, Any]) -> str:
    ...
    return (
        "🔍 PHÂN TÍCH\n"
        f"• Trạng thái: {report['mode']} | {report['note']}\n"
        f"• Sơ cấp: {ai.get('status', '-') } | {ai.get('confidence', 0)}%\n"
        f"• Sơ cấp PX: {format_prob_inline(probs)}\n"
        f"• Chốt cuối: {meta.get('final_label', '-') } | {meta.get('confidence', 0)}%\n"
        f"• Chốt PX: {format_prob_inline(final_probs)}\n"
        f"• Chuỗi: {report['last_label'] if report['last_label'] else '—'} x{report['streak']}\n"
        f"• Vòng lặp: {report['cycle_len'] if report['cycle_len'] else '—'} | {report['cycle_score']:.0f}\n"
        f"• Lịch sử: {hist}\n"
        f"• Pattern: {patterns}\n"
        f"• Lý do: {'; '.join(meta.get('reasons', [])[:3])}"
    )
...
async def delayed_analysis_job(chat_id: int, message, version: int) -> None:
    try:
        ...
        await message.reply_text(build_analysis_message(report, ai, meta, d))
        ...
        try:
            await message.reply_text("❌ Lỗi khi phân tích")
        except Exception:
            pass


async def send_delayed_analysis(update: Update, chat_id: int) -> None:
    ...
    task = asyncio.create_task(delayed_analysis_job(chat_id, message, version))
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            f"📊 THỐNG KÊ\n"
            f"• {LOW_LABEL}: {d['low_count']} | {HIGH_LABEL}: {d['high_count']}\n"
            f"• Cân bằng: {balance}% | Ổn định gần đây: {recent_stable}%\n"
            f"• Mode: {d.get('mode', 'WARMUP')} | Lượt: {d.get('updates', 0)} | Lịch sử: {hist_len}"
        )
    except Exception as e:
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        ai = forecast_next(chat_id, report, d.get("labels", []))
        meta = meta_ai_judge(chat_id, report, ai)
        await update.message.reply_text(build_analysis_message(report, ai, meta, d))
    except Exception as e:
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(build_stats_message(await load_user(chat_id)))
        await send_delayed_analysis(update, chat_id)
