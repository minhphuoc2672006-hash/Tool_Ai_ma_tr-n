...
def final_ai(history):
    ...
    # Nếu lệch quá nhiều, thêm xác suất ngẫu nhiên
    if counter[final]/len(preds) > 0.8:
        final = "Tài" if final=="Xỉu" else final

    conf = int(sum(confs)/len(confs))
    return final, conf

# ===== TRỰC QUAN THẮNG/THUA =====
def format_result_msg(n, tx, last_pred=None, win=False):
    robot_icon = "🤖"
    tick = "✅"
    cross = "❌"
    msg = f"{robot_icon} Kết quả: {n} ({tx})"
    if last_pred:
        if win:
            msg += f"\n{robot_icon} Dự đoán trước: {last_pred} → {tick} Thắng"
        else:
            msg += f"\n{robot_icon} Dự đoán trước: {last_pred} → {cross} Thua"
    return msg

def format_prediction_msg(pred, conf):
    robot_icon = "🤖"
    star = "✨"
    return f"{robot_icon} Dự đoán AI: {pred} {star}\nTỷ lệ: {conf}%"
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for n in nums:
        ...
        msg = format_result_msg(n, tx, data["last_pred"], win_flag)
    ...
    data["last_pred"] = pred
    await wait.edit_text(format_prediction_msg(pred, conf))
