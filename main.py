...
ANALYSIS_WINDOW_MIN = int(os.getenv("ANALYSIS_WINDOW_MIN", "25"))
ANALYSIS_WINDOW_MAX = int(os.getenv("ANALYSIS_WINDOW_MAX", "70"))
...
MIN_CONSENSUS_RATIO = float(os.getenv("MIN_CONSENSUS_RATIO", "0.60"))
MIN_FINAL_CONFIDENCE = int(os.getenv("MIN_FINAL_CONFIDENCE", "58"))
...
try:
    ...
    GROUP_CHAT_ID = None
    logger.warning("GROUP_CHAT_ID không hợp lệ: %s", GROUP_CHAT_ID_RAW)
...
def prune_history(conn: sqlite3.Connection, chat_id: int, keep_limit: int) -> None:
    if keep_limit <= 0:
        return
    row = conn.execute("SELECT id FROM history WHERE chat_id = ? ORDER BY id DESC LIMIT 1 OFFSET ?", (chat_id, max(0, keep_limit - 1))).fetchone()
    if row:
        ...
        try:
            with db_connect() as conn:
                for raw_value, label in items:
                    conn.execute("INSERT INTO history (chat_id, raw_value, label) VALUES (?, ?, ?)", (chat_id, int(raw_value), str(label)))
        ...
        try:
            with db_connect() as conn:
                if limit and limit > 0:
                    rows = conn.execute("SELECT raw_value, label FROM history WHERE chat_id = ? ORDER BY id DESC LIMIT ?", (chat_id, limit)).fetchall()
                    rows = list(reversed(rows))
                else:
                    rows = conn.execute("SELECT raw_value, label FROM history WHERE chat_id = ? ORDER BY id ASC", (chat_id,)).fetchall()
...
def new_state() -> Dict[str, Any]:
    return {
        ...
        "last_chart_msg_id": None,
        "last_robot_msg_id": None,
...
def parse_input(text: str) -> List[int]:
    ...
    if len(tokens) == 1:
        ...
        if len(tok) == 3 and all(ch in "123456" for ch in tok):
            return [sum(int(ch) for ch in tok)]
        try:
            n = int(tok)
    ...
    if all(1 <= int(x) <= 6 for x in tokens):
        ...
        while i + 3 <= len(tokens):
            rolls.append(sum(int(v) for v in tokens[i:i + 3]))
            i += 3
        while i < len(tokens):
            rolls.append(int(tokens[i]))
            i += 1
        return rolls[:MAX_INPUT_NUMS]
    out: List[int] = []
    for x in tokens:
        try:
            n = int(x)
...
def winrate_from_counts(wins: int, total: int) -> float:
    return safe_div(wins * 100.0, total)


def overall_winrate(state: Dict[str, Any]) -> float:
    return winrate_from_counts(int(state.get("prediction_hits", 0)), int(state.get("prediction_total", 0)))
...
def build_trace_message(state: Dict[str, Any]) -> str:
    ...
    patterns = state.get("last_model_predictions") or {}
    pattern_text = ", ".join(
        f"{k}:{v.get('label','-')}/{v.get('confidence',0)}%"
        for k, v in patterns.items()
    ) if isinstance(patterns, dict) and patterns else "-"
    return (
...
def choose_analysis_window(labels_full: List[str]) -> int:
    ...
    if probe_vol >= 0.72 or probe_ent >= 1.00 or (streak <= 2 and probe_vol >= 0.55):
        win = 70
    elif probe_vol <= 0.35 and probe_ent <= 0.82 and streak >= 4:
        win = 25
    elif probe_vol <= 0.48 and probe_ent <= 0.92:
        win = 35
    else:
        win = 50
...
def detect_transition_phase(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    prev_last, prev_streak = current_streak(prev)
    recent_last, recent_streak = current_streak(recent)
...
def detect_false_trap(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if tail[-5] != tail[-4] and tail[-4] == tail[-3] == tail[-1] and tail[-2] == tail[-3]:
        return {"name": "CẦU BẪY", "detail": "Nhịp ngắn có dấu hiệu gài", "score": 86, "hint": None}
    return None
...
def detect_all_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    return _dedupe_patterns(patterns)[:12]
...
def enter_loss_cooldown(state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> bool:
    ...
    state["model_accuracy"] = {"pattern": 50, "markov": 50, "chart": 50, "structure": 50}
...
def predict_pattern(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        if hint in (LOW_LABEL, HIGH_LABEL) and name in {
            ...
            "NGHIÊNG", "NGHIÊNG NHẸ", "XU HƯỚNG", "ĐẢO CHIỀU", "SÓNG ĐỀU", "SÓNG ĐẢO",
            "CẦU PHỨC HỢP", "CHU KỲ MỞ RỘNG", "CHUYỂN PHA", "XEN KẼ BIẾN THỂ",
        }:
...
def predict_structure(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        if name.startswith("CẦU ") or name.startswith("LẶP MẪU") or name.startswith("LẶP KHỐI") or name.startswith("SÓNG") or name.startswith("CHU KỲ") or name.startswith("CHUYỂN PHA") or name.startswith("XEN KẼ"):
            return {"label": hint if hint in (LOW_LABEL, HIGH_LABEL) else (labels[-1] if labels else LOW_LABEL), "confidence": min(92, int(top.get("score", 60)) + 1), "source": f"structure:{name}"}
        if name == "CÂN BẰNG":
...
def meta_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for name, pred in predictions.items():
        ...
        score *= 1.06 if name == "pattern" else 1.05 if name == "markov" else 1.02 if name == "structure" else 1.0
        if name == "chart":
            if abs(trend) >= 0.18:
                score *= 1.10
            elif abs(trend) >= 0.10:
    ...
    confidence = int(44 + top_ratio * 42 + (agreement - 1) * 5 + (strongest_conf - 50) * 0.15)
    if report.get("structure") in {"CÂN BẰNG", "TRUNG TÍNH", "CHƯA ĐỦ DỮ LIỆU"}:
        confidence -= 8
    if abs(trend) >= 0.18:
        confidence += 2
    confidence = max(0, min(confidence, 95))
    return {"model": best_model, "final_label": best_label, "confidence": confidence, "scores": dict(vote), "agreement": agreement, "top_ratio": top_ratio}


def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str]) -> Dict[str, Any]:
    ...
    if not allowed:
        ...
        state["last_model_predictions"] = {}
        return {"report": report, "adv": adv, "chart_features": chart_features, "predictions": {}, "meta": {}, "allowed": False, "reason": reason, "relearned": relearned, "resumed": resumed, "window": window, "labels_used": labels}
    ...
    if meta["top_ratio"] < MIN_CONSENSUS_RATIO or meta["agreement"] < 2 or meta["confidence"] < MIN_FINAL_CONFIDENCE:
        state["last_prediction_label"] = None
        state["last_prediction_conf"] = 0
        state["last_prediction_result"] = "TẠM DỪNG"
        state["last_note"] = f"Thiếu đồng thuận ({meta['agreement']}/{len(predictions)})"
        state["last_model_predictions"] = predictions
        return {"report": report, "adv": adv, "chart_features": chart_features, "predictions": predictions, "meta": meta, "allowed": False, "reason": f"Thiếu đồng thuận ({meta['agreement']}/{len(predictions)})", "relearned": relearned, "resumed": resumed, "window": window, "labels_used": labels}
    ...
    return {"report": report, "adv": adv, "chart_features": chart_features, "predictions": predictions, "meta": meta, "allowed": True, "reason": reason, "relearned": relearned, "resumed": resumed, "window": window, "labels_used": labels}
...
def build_bridge_chart_image(labels: List[str], report: Dict[str, Any], adv: Dict[str, Any], state: Optional[Dict[str, Any]] = None, limit: int = 0) -> Optional[BytesIO]:
    ...
    ax.text(0.98, 0.02, f"Cầu: {quick}", transform=ax.transAxes, va="bottom", ha="right", fontsize=9, color="white", bbox=dict(boxstyle="round,pad=0.35", facecolor="#1f2937", alpha=0.9, edgecolor="#6b7280"))
    ax.text(0.02, 0.02, "Sóng xanh = ngắn hạn | Sóng vàng = trung hạn", transform=ax.transAxes, va="bottom", ha="left", fontsize=8.5, color="white", bbox=dict(boxstyle="round,pad=0.30", facecolor="#1f2937", alpha=0.82, edgecolor="#6b7280"))
    fig.tight_layout()
    buf = BytesIO()
...
async def send_bridge_chart(update: Update, labels: List[str], report: Dict[str, Any], adv: Dict[str, Any], title: str, state: Optional[Dict[str, Any]] = None) -> Optional[Any]:
    ...
    try:
        chart.seek(0)
        return await update.message.reply_photo(photo=chart, caption=f"{title}")
    except Exception as e:
...
def build_stats_message(report: Dict[str, Any], state: Dict[str, Any], adv: Dict[str, Any]) -> str:
    ...
    model_acc = state.get("model_accuracy", {})
    model_acc_line = f"P:{int(model_acc.get('pattern', 50))}% M:{int(model_acc.get('markov', 50))}% C:{int(model_acc.get('chart', 50))}% S:{int(model_acc.get('structure', 50))}%"
...
async def send_group_message(context: ContextTypes.DEFAULT_TYPE, text: str) -> None:
    if GROUP_CHAT_ID is None:
        return
    try:
        await context.bot.send_message(chat_id=GROUP_CHAT_ID, text=text)
    except Exception as e:
        logger.exception("send_group_message failed: %s", e)


def build_group_result_message(state: Dict[str, Any]) -> str:
    result = state.get("last_prediction_result", "CHƯA RÕ")
    if result == "ĐÚNG":
        head = "✅ DỰ ĐOÁN ĐÚNG"
    elif result == "SAI":
        head = "❌ DỰ ĐOÁN SAI"
    ...
    return (
        f"{head}\n"
        f"Tổng số phiên: {state.get('prediction_total', 0)}\n"
        f"Thắng: {state.get('prediction_hits', 0)}\n"
        f"Thua: {state.get('prediction_misses', 0)}"
    )


def build_group_analysis_message(state: Dict[str, Any], result: Dict[str, Any]) -> str:
    meta = result.get("meta", {}) if isinstance(result, dict) else {}
    report = result.get("report", {}) if isinstance(result, dict) else {}
    if not result.get("allowed", False):
        return (
            "📊 PHÂN TÍCH\n"
            f"Trạng thái: TẠM DỪNG\n"
            f"Lý do: {result.get('reason', '-')}"
        )
    final_label = meta.get("final_label") or state.get("last_prediction_label") or "-"
    confidence = meta.get("confidence", state.get("last_prediction_conf", 0))
    return (
        "📊 PHÂN TÍCH\n"
        f"Cầu: {state.get('last_detected_pattern') or '-'}\n"
        f"Cấu trúc: {state.get('last_structure') or report.get('structure', '-')}\n"
        f"Hướng: {state.get('last_detected_hint') or '-'}\n"
        f"Chốt tiếp: {final_label}\n"
        f"Độ tin cậy: {confidence}%"
    )


async def safe_delete_message(msg, delay: float = 0.0) -> None:
    if not msg:
        return
    try:
        if delay > 0:
        ...
        logger.debug("Không xoá được message: %s", e)
...
async def send_robot_analysis_sequence(update: Update, meta: Dict[str, Any], state: Dict[str, Any], cleanup_messages: Optional[List[Any]] = None) -> None:
    msg = await send_robot_status(update, "🤖 ĐANG PHÂN TÍCH...")
    if msg:
        state["last_robot_msg_id"] = msg.message_id
    ...
    if is_group_chat(update) and cleanup_messages:
        for m in cleanup_messages:
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    mirror_to_group = GROUP_CHAT_ID is not None and GROUP_CHAT_ID != chat_id
    ...
    async with STATE_LOCK:
        ...
        if entries:
            ...
            group_result_text = build_group_result_message(state)
    ...
    if not mirror_to_group:
        chart_msg = await send_bridge_chart(update, labels_used, report, adv, "📈 BIỂU ĐỒ CẦU - CẬP NHẬT MỚI", state)
        if chart_msg:
            state["last_chart_msg_id"] = chart_msg.message_id

        if not result.get("allowed", False):
            robot_msg = await send_robot_status(update, f"⏸ BOT TẠM DỪNG\nLý do: {result.get('reason', 'Cầu chưa rõ')}")
            if robot_msg:
                state["last_robot_msg_id"] = robot_msg.message_id
            return

        meta = result["meta"]
        await send_robot_analysis_sequence(update, meta, state, cleanup_messages=[chart_msg] if chart_msg else None)
    else:
        if entries:
            await send_group_message(context, group_result_text or build_group_result_message(state))
        await send_group_message(context, build_group_analysis_message(state, result))
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"/ai - phân tích cầu\n"
        f"/next - giống /ai\n"
        f"/why - xem lý do chốt/skip gần nhất\n"
        ...
        f"Luồng hoạt động: cập nhật thống kê → biểu đồ → robot phân tích → chốt cuối."
    )
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(build_stats_message(report_full, state, adv_full))
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if GROUP_CHAT_ID is not None and GROUP_CHAT_ID == chat_id:
        try:
            await send_group_message(context, build_group_result_message(state))
        except Exception as e:
            logger.exception("Gửi result group failed: %s", e)
        try:
            await send_group_message(context, build_group_analysis_message(state, result))
        except Exception as e:
            logger.exception("Gửi analysis group failed: %s", e)
        return

    chart_msg = await send_bridge_chart(update, labels_used, report, adv, "📈 BIỂU ĐỒ CẦU - DÙNG CHO PHÂN TÍCH", state)
    if chart_msg:
        state["last_chart_msg_id"] = chart_msg.message_id
