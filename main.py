...
def final_decision(
    ...
    if recheck_status == "RECHECK_GHOST":
        ...
        w_r, w_g = 0.66, 0.34
    elif white_type in ("CẦU TRẮNG KHÁNG ÁM", "CẦU TRẮNG BỊ ÁM", "CẦU CHUYỂN PHA") or ghost_score >= DRIFT_MIN_SHIFT * 100 or shift_score >= DRIFT_MIN_SHIFT:
        if timing_state == "EARLY":
            w_r, w_g = 0.45, 0.55
        elif timing_state == "LATE":
            w_r, w_g = 0.72, 0.28
    ...
    if timing_state == "EARLY":
        ...
        scores[LOW_LABEL] = scores[LOW_LABEL] * 1.01
        scores[HIGH_LABEL] = scores[HIGH_LABEL] * 1.01
    ...
    delta = abs(probs[LOW_LABEL] - probs[HIGH_LABEL])
    confidence = int(max(0, min(100, (rhythm_ai.get("confidence", 0) * 0.34) + (ghost_ai.get("confidence", 0) * 0.34) + (recheck_conf * 0.12) + (timing.get("confidence", 0) * 0.12) + delta * 24 + (5 if stability >= 70 else 0) - (8 if stability < 45 else 0))))
    if timing_state == "EARLY":
    ...
    return {
        ...
        "timing_confidence": timing.get("confidence", 0),
...
def build_final_message(meta: Dict[str, Any]) -> str:
    return (
        ...
        f"TIMING: {meta.get('timing_state', '-')}\n"
        f"GHI CHÚ: {meta.get('timing_note', '-')}"
    )
