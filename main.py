...
async def load_user(chat_id: int) -> Dict[str, Any]:
    ...
    async with DB_LOCK:
        row, hist_rows, pat_rows = await asyncio.to_thread(lambda: safe_db_call(_work))
...
async def save_user(chat_id: int, d: Dict[str, Any]) -> None:
    async with DB_LOCK:
        ...
        await asyncio.to_thread(lambda: safe_db_call(_work))
...
def detect_mode(d: Dict[str, Any], report: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if ghost_score >= GHOST_SOFT_CLEAN and shift_score >= WHITE_SHIFT_THRESHOLD:
...
def ai_level2_decision(d: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    recent_probs = weighted_label_probs(recent_labels, window=max(8, min(RECENT_WINDOW, len(recent_labels))), decay=0.90)
    ...
    if len(labels) < MIN_ANALYSIS_LEN:
        return pack(recent_best, 0, "Chưa đủ dữ liệu.", "WARMUP")

    if cau_type == "CẦU XEN KẼ":
        if last_label in (LOW_LABEL, HIGH_LABEL):
            return pack(opposite(last_label), 82, "Cầu xen kẽ: bẻ theo nhịp đảo.", "CẦU XEN KẼ")

    if cau_type == "CẦU BỆT":
        if last_label in (LOW_LABEL, HIGH_LABEL):
            return pack(last_label, 84, f"Cầu bệt: theo {last_label}.", "CẦU BỆT")
    ...
    if cau_type == "CẦU TRẮNG KHÁNG ÁM":
        return pack(recent_best, 78 if recent_gap >= 0.06 else 70, "Cầu trắng kháng ám: ưu tiên nhịp mới sạch.", cau_type)

    if cau_type == "CẦU TRẮNG BỊ ÁM":
        return pack(recent_best, 70 if recent_gap >= 0.06 else 64, "Cầu trắng bị ám: đọc đuôi mới nhưng thận trọng.", cau_type)

    if cau_type == "CẦU TRẮNG":
        return pack(recent_best, 72 if recent_gap >= 0.06 else 66, "Cầu trắng: ưu tiên tín hiệu gần nhất.", cau_type)

    if cau_type == "CẦU RUNG":
        if recent_gap >= 0.10:
            return pack(recent_best, 66, "Cầu rung: dùng đuôi gần nhất.", cau_type)
        if last_label in (LOW_LABEL, HIGH_LABEL):
            return pack(opposite(last_label), 62, "Cầu rung: ưu tiên đảo nhịp ngắn hạn.", cau_type)

    if cau_type == "CẦU HỖN LOẠN":
        if recent_gap >= 0.08:
            return pack(recent_best, 60, "Cầu hỗn loạn: chọn tín hiệu gần hơn.", cau_type)
        if last_label in (LOW_LABEL, HIGH_LABEL):
            return pack(last_label, 56, "Cầu hỗn loạn: bám nhịp mới nhất.", cau_type)

    if cau_type == "CẦU ỔN ĐỊNH":
        if last_label in (LOW_LABEL, HIGH_LABEL) and streak >= 3:
            return pack(last_label, 75, "Cầu ổn định: bám nhịp đang chạy.", cau_type)
    ...
    return pack(LOW_LABEL, 50, "Cầu chưa rõ.", "CHƯA RÕ CẦU")
...
def final_decision(report: Dict[str, Any], d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    meta["final_probs"] = {
        LOW_LABEL: 1.0 if meta["final_label"] == LOW_LABEL else 0.0,
        HIGH_LABEL: 1.0 if meta["final_label"] == HIGH_LABEL else 0.0,
    }
    meta["other_label"] = HIGH_LABEL if meta["final_label"] == LOW_LABEL else LOW_LABEL
    return meta
...
def build_final_message(meta: Dict[str, Any]) -> str:
    return (
        ...
        f"XÁC SUẤT: {format_prob_inline(meta.get('final_probs', {LOW_LABEL: 0.5, HIGH_LABEL: 0.5}))}"
    )
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    async with STATE_LOCK:
        ...
        if nums:
            ...
            # cập nhật thống kê NGAY trên RAM trước, rồi mới phân tích
            rebuild_counters(d)
            trim_state_memory(d)

        # phân tích dựa trên dữ liệu đã cập nhật trong RAM
        ...
        # lưu DB sau khi đã có thống kê + phân tích từ RAM
        await persist_snapshot(chat_id, d, entries)

    await update.message.reply_text("✅ Đã cập nhật bảng thống kê. Phân tích ngay lập tức...")
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            ...
            "Bot chốt theo cầu + nhịp gần nhất, không dùng tổng lịch sử làm điểm chốt chính."
        )
    except Exception as e:
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # đảm bảo dữ liệu đã được cập nhật xong trước khi phân tích
        report = analyze_sequence(d)
        cleanup = anti_ghost_cleanup(d, report)
        repair_state(d)
        report = analyze_sequence(d)
