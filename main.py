...
def table_stat_ai(labels: List[str], ghost_info: Dict[str, Any], trap_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Quy tắc chính:
    # - Lặp thật: chu kỳ rõ, độ thuần cao, đuôi ổn.
    # - Lặp giả: chu kỳ có nhưng đuôi đổi pha, nhiễu, hoặc bị kéo lệch.
    # - Không dùng tổng đếm để quyết định.
    real_points = 0.0
    fake_points = 0.0
...
def detect_cau_structure(report: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if table_verdict == "LẶP GIẢ" or (trap_is_lure and trap_repeat_score >= 60):
        return "CẦU LẶP GIẢ", "AI bảng thống kê chốt: lặp giả / lừa lặp"
    if table_verdict == "LẶP THẬT":
        return "CẦU LẶP", "AI bảng thống kê chốt: lặp thật"
...
def detect_table_repeat_ai_for_report(d: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, Any]:
    return table_stat_ai(d.get("labels", []), report, {
        "trap_repeat_score": report.get("trap_repeat_score", 0.0),
        "trap_is_lure": report.get("trap_is_lure", False),
        "trap_purity": report.get("trap_purity", 0.0),
    })
...
def ai_level2_decision(d: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trap_observed = report.get("trap_observed")
    table_verdict = report.get("table_ai_verdict", "CHƯA RÕ")
    ...
    # Tầng đầu: bảng thống kê chốt thật/giả trước.
    if table_verdict == "LẶP GIẢ":
        target = opposite(trap_expected) if trap_expected in (LOW_LABEL, HIGH_LABEL) else opposite(last_label)
        if target not in (LOW_LABEL, HIGH_LABEL):
            target = recent_best
        conf = 80 if trap_score >= 75 else 74
        if recent_gap >= 0.08:
            conf += 4
        meta = pack(target, conf, "AI bảng thống kê chốt: lặp giả, ưu tiên bẻ.", "CẦU LẶP GIẢ")
    ...
    if table_verdict == "LẶP THẬT":
        ...
        if cycle_pred in (LOW_LABEL, HIGH_LABEL):
            ...
            meta = pack(cycle_pred, conf, "AI bảng thống kê chốt: lặp thật, theo chu kỳ.", "CẦU LẶP THẬT")
    ...
    if trap_is_lure and trap_score >= 60:
        ...
        if recent_gap >= 0.08:
            conf += 4
        meta = pack(target, conf, "Lặp giả: mẫu cũ đang dụ đảo, ưu tiên bẻ.", "CẦU LẶP GIẢ")
        meta["decision_hint"] = "BẺ"
    ...
    # không lấy tổng đếm làm quyết định chính, chỉ xem nhịp gần
    if recent_gap < 0.04 and turn_score < 0.12 and early_shift_score < 0.12:
        if last_label in (LOW_LABEL, HIGH_LABEL):
            meta = pack(last_label, 56, "Cầu chưa rõ: giữ nhịp gần nhất.", "CHƯA RÕ CẦU")
            meta["decision_hint"] = "TRUNG_LẬP"
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    async with STATE_LOCK:
        ...
        # Tầng 1: nhìn bảng thống kê trước
        ghost_preview = detect_ghost_pressure(d.get("labels", []))
        trap_preview = detect_lure_repeat(d.get("labels", []))
        table_ai = table_stat_ai(d.get("labels", []), ghost_preview, trap_preview)

        report = analyze_sequence(d)
        report["table_ai_verdict"] = table_ai["table_ai_verdict"]
        report["table_ai_reason"] = table_ai["table_ai_reason"]
        report["table_ai_score"] = table_ai["table_ai_score"]

        cleanup = anti_ghost_cleanup(d, report)
        repair_state(d)

        # Lần 2: phân tích sau khi AI bảng thống kê đã chốt
        report = analyze_sequence(d)
        report["table_ai_verdict"] = table_ai["table_ai_verdict"]
        report["table_ai_reason"] = table_ai["table_ai_reason"]
        report["table_ai_score"] = table_ai["table_ai_score"]
