...
def meta_ai_judge(report: Dict[str, Any], ai: Dict[str, Any]) -> Dict[str, Any]:
    """
    AI cấp 2: đọc report + ai sơ cấp rồi chốt riêng lẻ Tài/Xỉu.
    """
    base_label = ai.get("best_label")
    if base_label not in (LOW_LABEL, HIGH_LABEL):
        base_label = HIGH_LABEL if int(report.get("high", 0)) >= int(report.get("low", 0)) else LOW_LABEL
    ...
    if long_mode.startswith("IMBALANCED") and long_score >= 2:
        final_label = LOW_LABEL if low > high else HIGH_LABEL
        confidence += 6
        reasons.append("lệch tổng thể rõ")
    ...
    return {
        ...
        "confidence": confidence,
        "reasons": reasons if reasons else ["tổng hợp từ report hiện tại"],
    }
...
# RENDER
# =========================
def build_analysis_output(report: Dict[str, Any], ai: Dict[str, Any], hist: str, pattern_text: str) -> str:
...
def build_final_ai_output(report: Dict[str, Any], ai: Dict[str, Any], meta: Dict[str, Any]) -> str:
    final_label = meta.get("final_label", "-")
    final_conf = int(meta.get("confidence", 0))
    reasons = meta.get("reasons", [])
    reason_text = " • ".join(reasons[:3]) if reasons else "tổng hợp tín hiệu hiện tại"

    return f"""
🧠 AI CẤP 2 - CHỐT CUỐI

Kết luận: {final_label}
...
Tín hiệu nền:
• Trạng thái sơ cấp: {ai.get('status', '-') }
• Đoạn nhận xét    : {ai.get('message', '-')}

Gợi ý:
• AI này đọc lại report vừa phân tích rồi mới chốt.
• Không phải kết quả tuyệt đối, nhưng ưu tiên tín hiệu mạnh nhất hiện có.
""".strip()
...
╔══════════════📘 TRỢ GIÚP══════════════╗
/stats - xem thống kê
/ai - xem phân tích + AI chốt riêng lẻ
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        meta = meta_ai_judge(report, ai)

        patterns = report.get("patterns", [])
        pattern_text = format_pattern_lines(patterns)
        hist = format_history(d.get("labels", []))

        # Tin nhắn 1: bản phân tích
        await update.message.reply_text(build_analysis_output(report, ai, hist, pattern_text))

        # Tin nhắn 2: AI cấp 2 chốt riêng
        await asyncio.sleep(0.05)
        await update.message.reply_text(build_final_ai_output(report, ai, meta))
    except Exception as e:
