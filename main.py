        ...
        with db_connect() as conn:
            row = conn.execute(
                ...
                SELECT low_count, high_count, mode, ghost_mode, updates,
                       ...
                       health_log_json, monitor_log_json
...
def master_controller(d: Dict[str, Any], ghost_info: Dict[str, Any]) -> Tuple[str, str]:
    labels = d.get("labels", [])
    if len(labels) < 10:
        return "WARMUP", "Đang học dữ liệu"

    white_type = ghost_info.get("white_type", "CHƯA RÕ CẦU TRẮNG")
    white_score = float(ghost_info.get("white_score", 0.0))
...
def analyze_sequence(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mode, note = master_controller(d, ghost_info)
...
def ai_new_rhythm(chat_id: int, report: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    scores = {
        LOW_LABEL: recent_probs[LOW_LABEL] * 0.52 + mid_probs[LOW_LABEL] * 0.16 + markov_probs[LOW_LABEL] * 0.22 + global_probs[LOW_LABEL] * 0.10,
        HIGH_LABEL: recent_probs[HIGH_LABEL] * 0.52 + mid_probs[HIGH_LABEL] * 0.16 + markov_probs[HIGH_LABEL] * 0.22 + global_probs[HIGH_LABEL] * 0.10,
    }
    ...
    if cycle_score >= 60 and cycle_len:
        ...
        if cycle_pred in (LOW_LABEL, HIGH_LABEL):
            scores[cycle_pred] += conf / 100.0 * 0.10
    if vol >= 0.65:
...
def monitor_ai(chat_id: int, d: Dict[str, Any], report: Dict[str, Any], rhythm_ai: Dict[str, Any], ghost_ai: Dict[str, Any]) -> Dict[str, Any]:
    labels = d.get("labels", [])
    total = len(labels)
    ghost_score = float(report.get("ghost_score", 0.0))
    shift_score = float(report.get("shift_score", 0.0))
    ...
    stability = 100.0
    stability -= ghost_score * 0.55
    stability -= shift_score * 40.0
    stability += freshness * 8.0
    stability += 4.0 if mode in ("NORMAL", "BALANCED", "WHITE_CLEAN") else 0.0
    stability -= 8.0 if mode == "NOISY" else 0.0
...
def final_ensemble(chat_id: int, report: Dict[str, Any], rhythm_ai: Dict[str, Any], ghost_ai: Dict[str, Any], monitor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    severity = monitor.get("severity", "LOW")

    if white_type in ("CẦU TRẮNG KHÁNG ÁM", "CẦU TRẮNG BỊ ÁM", "CẦU CHUYỂN PHA") or ghost_score >= 65 or shift_score >= DRIFT_MIN_SHIFT:
        w_r, w_g = 0.74, 0.26
    elif mode == "NOISY":
        w_r, w_g = 0.55, 0.45
    else:
    ...
    if white_type == "CẦU TRẮNG KHÁNG ÁM":
        scores[LOW_LABEL] += 0.03
        scores[HIGH_LABEL] += 0.03
    ...
    confidence = int(
        max(
            ...
            min(
                100,
                (rhythm_ai.get("confidence", 0) * 0.42)
                + (ghost_ai.get("confidence", 0) * 0.42)
                + 8
                + delta * 28
    ...
    reasons = [
        f"AI nhịp mới: {rhythm_ai.get('status', rhythm_ai.get('engine', '-'))}",
        f"AI chống ám: {ghost_ai.get('status', ghost_ai.get('engine', '-'))}",
        f"Giám sát: {severity}",
    ]

    if white_type in ("CẦU TRẮNG KHÁNG ÁM", "CẦU TRẮNG BỊ ÁM", "CẦU CHUYỂN PHA"):
        reasons.append(white_type)
    if ghost_score >= 65:
        reasons.append("lọc ám lịch sử")
    if freshness >= 0.70:
        reasons.append("ưu tiên dữ liệu mới")
    if stability < 45:
        reasons.append("hạ niềm tin vì độ ổn định thấp")
...
def build_stats_message(report: Dict[str, Any], d: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"║ Stable: {d.get('stability_score', 100.0):.1f}\n"
        "╠════════════════════════════╣\n"
...
def build_loading_message(report: Dict[str, Any], rhythm_ai: Dict[str, Any], ghost_ai: Dict[str, Any], monitor: Dict[str, Any]) -> str:
    return (
        "╔════════════════════════════╗\n"
        "║      ⏳ ĐANG PHÂN TÍCH      ║\n"
        "╠════════════════════════════╣\n"
        ...
        "║ Hệ thống đang khóa tín hiệu mới và lọc ám cũ...\n"
        "╚════════════════════════════╝"
    )


def build_analysis_message(report: Dict[str, Any], rhythm_ai: Dict[str, Any], ghost_ai: Dict[str, Any], meta: Dict[str, Any], monitor: Dict[str, Any]) -> str:
    return (
        "╔════════════════════════════╗\n"
        "║       🔍 PHÂN TÍCH         ║\n"
        "╠════════════════════════════╣\n"
...
async def delayed_analysis_job(chat_id: int, version: int, loading_msg) -> None:
    try:
        ...
        monitor = monitor_ai(chat_id, d, report, rhythm_ai, ghost_ai)
        meta = final_ensemble(chat_id, report, rhythm_ai, ghost_ai, monitor)

        if analysis_versions.get(chat_id, 0) != version:
            return

        try:
            await loading_msg.edit_text(build_analysis_message(report, rhythm_ai, ghost_ai, meta, monitor))
        except Exception:
            pass
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            ...
            f"Bot dùng 3 tầng: AI nhịp mới, AI chống ám, AI giám sát.\n"
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        ghost_ai = ai_anti_ghost(chat_id, report, d.get("labels", []))
        monitor = monitor_ai(chat_id, d, report, rhythm_ai, ghost_ai)
        _ = final_ensemble(chat_id, report, rhythm_ai, ghost_ai, monitor)

        await update.message.reply_text(build_stats_message(report, d))
        loading_msg = await update.message.reply_text(build_loading_message(report, rhythm_ai, ghost_ai, monitor))
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if update.message:
            ...
            monitor = monitor_ai(chat_id, d, report_now, rhythm_ai, ghost_ai)
            ...
            loading_msg = await update.message.reply_text(
                build_loading_message(report_now, rhythm_ai, ghost_ai, monitor)
            )
            await start_delayed_analysis(context, chat_id, loading_msg)
