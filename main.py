...
def new_user() -> Dict[str, Any]:
    return {
        ...
        "timing_early_miss": 0,
        "timing_late_miss": 0,
        "timing_last_gate": 58.0,
        "timing_last_index": 0.0,
        "timing_last_state": "BALANCED",
        "prediction_total": 0,
        "prediction_hits": 0,
        "prediction_misses": 0,
        "flip_total": 0,
        ...
        "last_action_result": "CHƯA RÕ",
        "last_timing_fault": "NONE",
        "last_decision_note": "",
...
def repair_state(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    d.setdefault("timing_bias", 0.0)
    d.setdefault("timing_hits", 0)
    d.setdefault("timing_misses", 0)
    d.setdefault("timing_early_miss", 0)
    d.setdefault("timing_late_miss", 0)
    d.setdefault("timing_last_gate", 58.0)
    d.setdefault("timing_last_index", 0.0)
    d.setdefault("timing_last_state", "BALANCED")
    d.setdefault("prediction_total", 0)
    d.setdefault("prediction_hits", 0)
    d.setdefault("prediction_misses", 0)
    d.setdefault("flip_total", 0)
    d.setdefault("flip_hits", 0)
    d.setdefault("flip_misses", 0)
    d.setdefault("follow_total", 0)
    d.setdefault("follow_hits", 0)
    d.setdefault("follow_misses", 0)
    d.setdefault("last_prediction_result", "CHƯA RÕ")
    d.setdefault("last_action_result", "CHƯA RÕ")
    d.setdefault("last_timing_fault", "NONE")
...
async def load_user(chat_id: int, force_reload: bool = False) -> Dict[str, Any]:
    ...
    if row:
        ...
        try:
            ...
            state["timing_bias"] = float(meta.get("timing_bias", 0.0) or 0.0)
            state["timing_hits"] = int(meta.get("timing_hits", 0) or 0)
            state["timing_misses"] = int(meta.get("timing_misses", 0) or 0)
            state["timing_early_miss"] = int(meta.get("timing_early_miss", 0) or 0)
            state["timing_late_miss"] = int(meta.get("timing_late_miss", 0) or 0)
            state["timing_last_gate"] = float(meta.get("timing_last_gate", 58.0) or 58.0)
            state["timing_last_index"] = float(meta.get("timing_last_index", 0.0) or 0.0)
            state["timing_last_state"] = meta.get("timing_last_state", "BALANCED")
            state["prediction_total"] = int(meta.get("prediction_total", 0) or 0)
            state["prediction_hits"] = int(meta.get("prediction_hits", 0) or 0)
            state["prediction_misses"] = int(meta.get("prediction_misses", 0) or 0)
            state["flip_total"] = int(meta.get("flip_total", 0) or 0)
            state["flip_hits"] = int(meta.get("flip_hits", 0) or 0)
            state["flip_misses"] = int(meta.get("flip_misses", 0) or 0)
            state["follow_total"] = int(meta.get("follow_total", 0) or 0)
            state["follow_hits"] = int(meta.get("follow_hits", 0) or 0)
            state["follow_misses"] = int(meta.get("follow_misses", 0) or 0)
            state["last_prediction_result"] = meta.get("last_prediction_result", "CHƯA RÕ")
            state["last_action_result"] = meta.get("last_action_result", "CHƯA RÕ")
            state["last_timing_fault"] = meta.get("last_timing_fault", "NONE")
        except Exception:
            pass
            ...
            def _work():
                with db_connect() as conn:
                    meta = {
                        ...
                        "timing_bias": float(d.get("timing_bias", 0.0) or 0.0),
                        "timing_hits": int(d.get("timing_hits", 0) or 0),
                        "timing_misses": int(d.get("timing_misses", 0) or 0),
                        "timing_early_miss": int(d.get("timing_early_miss", 0) or 0),
                        "timing_late_miss": int(d.get("timing_late_miss", 0) or 0),
                        "timing_last_gate": float(d.get("timing_last_gate", 58.0) or 58.0),
                        "timing_last_index": float(d.get("timing_last_index", 0.0) or 0.0),
                        "timing_last_state": d.get("timing_last_state", "BALANCED"),
                        "prediction_total": int(d.get("prediction_total", 0) or 0),
                        "prediction_hits": int(d.get("prediction_hits", 0) or 0),
                        "prediction_misses": int(d.get("prediction_misses", 0) or 0),
                        "flip_total": int(d.get("flip_total", 0) or 0),
                        "flip_hits": int(d.get("flip_hits", 0) or 0),
                        "flip_misses": int(d.get("flip_misses", 0) or 0),
                        "follow_total": int(d.get("follow_total", 0) or 0),
                        "follow_hits": int(d.get("follow_hits", 0) or 0),
                        "follow_misses": int(d.get("follow_misses", 0) or 0),
                        "last_prediction_result": d.get("last_prediction_result", "CHƯA RÕ"),
                        "last_action_result": d.get("last_action_result", "CHƯA RÕ"),
                        "last_timing_fault": d.get("last_timing_fault", "NONE"),
                    }
                    conn.execute(
                        """
                        INSERT INTO users (
...
def _is_valid_label(label: Optional[str]) -> bool:
    return label in (LOW_LABEL, HIGH_LABEL)


def _root_label_from_context(
    recent_best: Optional[str],
    last_label: Optional[str],
) -> str:
    if _is_valid_label(recent_best):
        return recent_best
    if _is_valid_label(last_label):
        return last_label
    return LOW_LABEL


def _timing_allows_flip(report: Dict[str, Any]) -> bool:
    ...
    # Nới ngưỡng để bẻ sớm hơn, tránh bị giữ theo đuôi cũ quá lâu
    return (
        turn_score >= 0.10
        or early_shift_score >= 0.09
        or shift_score >= max(0.18, WHITE_SHIFT_THRESHOLD * 0.75)
        or (white_score >= 50.0 and ghost_score >= GHOST_SOFT_CLEAN * 0.75)
    )
...
def update_timing_feedback(d: Dict[str, Any], actual_label: str) -> None:
    """Nhận diện dự đoán đúng/sai và bẻ đúng/sai để tự điều chỉnh dần."""
    if actual_label not in (LOW_LABEL, HIGH_LABEL):
        return

    pred = d.get("last_prediction_label")
    if pred not in (LOW_LABEL, HIGH_LABEL):
        return

    hint = d.get("last_decision_hint", "TRUNG_LẬP")
    prev_actual = d.get("labels", [])[-2] if len(d.get("labels", [])) >= 2 else None
    last_conf = int(d.get("last_prediction_conf", 0) or 0)
    weight = _clamp(0.35 + last_conf / 150.0, 0.35, 0.95)

    correct = (pred == actual_label)

    d["prediction_total"] = int(d.get("prediction_total", 0)) + 1
    if correct:
        d["prediction_hits"] = int(d.get("prediction_hits", 0)) + 1
        d["last_prediction_result"] = "ĐÚNG"
    else:
        d["prediction_misses"] = int(d.get("prediction_misses", 0)) + 1
        d["last_prediction_result"] = "SAI"

    if hint == "BẺ":
        d["flip_total"] = int(d.get("flip_total", 0)) + 1
        if correct:
            d["flip_hits"] = int(d.get("flip_hits", 0)) + 1
            d["last_action_result"] = "BẺ ĐÚNG"
            d["last_timing_fault"] = "NONE"
            d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.95 + 0.02 * weight
        else:
            d["flip_misses"] = int(d.get("flip_misses", 0)) + 1
            if prev_actual in (LOW_LABEL, HIGH_LABEL) and actual_label == prev_actual:
                d["timing_early_miss"] = int(d.get("timing_early_miss", 0)) + 1
                d["last_action_result"] = "BẺ SAI / QUÁ SỚM"
                d["last_timing_fault"] = "EARLY"
                d["timing_bias"] = float(d.get("timing_bias", 0.0)) - (0.11 * weight)
            elif prev_actual in (LOW_LABEL, HIGH_LABEL) and actual_label == opposite_label(prev_actual):
                d["last_action_result"] = "BẺ SAI / LỆCH GỐC"
                d["last_timing_fault"] = "WRONG_DIR"
                d["timing_bias"] = float(d.get("timing_bias", 0.0)) - (0.04 * weight)
            else:
                d["last_action_result"] = "BẺ SAI"
                d["last_timing_fault"] = "MISS"
                d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.96

    elif hint == "THEO":
        d["follow_total"] = int(d.get("follow_total", 0)) + 1
        if correct:
            d["follow_hits"] = int(d.get("follow_hits", 0)) + 1
            d["last_action_result"] = "THEO ĐÚNG"
            d["last_timing_fault"] = "NONE"
            d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.95 - 0.01 * weight
        else:
            d["follow_misses"] = int(d.get("follow_misses", 0)) + 1
            if prev_actual in (LOW_LABEL, HIGH_LABEL) and actual_label == opposite_label(prev_actual):
                d["timing_late_miss"] = int(d.get("timing_late_miss", 0)) + 1
                d["last_action_result"] = "THEO SAI / QUÁ TRỄ"
                d["last_timing_fault"] = "LATE"
                d["timing_bias"] = float(d.get("timing_bias", 0.0)) + (0.11 * weight)
            elif prev_actual in (LOW_LABEL, HIGH_LABEL) and actual_label == prev_actual:
                d["last_action_result"] = "THEO SAI / LỆCH GỐC"
                d["last_timing_fault"] = "WRONG_DIR"
                d["timing_bias"] = float(d.get("timing_bias", 0.0)) + (0.04 * weight)
            else:
                d["last_action_result"] = "THEO SAI"
                d["last_timing_fault"] = "MISS"
                d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.96

    else:
        d["last_action_result"] = "TRUNG LẬP"
        d["last_timing_fault"] = "NONE"
        if correct:
            d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.98
        else:
            d["timing_bias"] = float(d.get("timing_bias", 0.0)) * 0.97

    d["timing_bias"] = _clamp(float(d.get("timing_bias", 0.0)), -1.0, 1.0)


def timing_ai_governor(d: Dict[str, Any], report: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    """AI điều phối thời điểm bẻ để giảm lỗi bẻ sớm / bẻ trễ."""
    ...
    white_score = float(report.get("white_score", 0.0))
    ghost_score = float(report.get("ghost_score", 0.0))
    final_probs = meta.get("final_probs", {LOW_LABEL: 0.5, HIGH_LABEL: 0.5})
    recent_gap = abs(float(final_probs.get(LOW_LABEL, 0.5)) - float(final_probs.get(HIGH_LABEL, 0.5)))

    bias = float(d.get("timing_bias", 0.0))
    # bias > 0: đang hay bẻ trễ => cho phép bẻ sớm hơn
    # bias < 0: đang hay bẻ sớm => siết lại
    base_gate = 58.0
    gate = _clamp(base_gate - (bias * 12.0), 42.0, 72.0)

    flip_index = (
        trap_score * 0.28
        + shift_score * 100.0 * 0.20
        + early_shift_score * 100.0 * 0.18
        + turn_score * 100.0 * 0.20
        + max(0.0, white_score - 45.0) * 0.10
        + max(0.0, ghost_score - 55.0) * 0.05
    )
    flip_index -= max(0.0, 0.10 - recent_gap) * 120.0
    flip_index = _clamp(flip_index, 0.0, 100.0)

    state = "WAIT"
    adjusted_hint = meta.get("decision_hint", "TRUNG_LẬP")
    adjusted_note = meta.get("decision_note", "")

    if flip_index >= gate + 10:
        state = "FORCE_FLIP"
        adjusted_hint = "BẺ"
        adjusted_note = "AI điều phối: tín hiệu đủ mạnh nên cho bẻ ngay để tránh bẻ trễ."
    elif flip_index >= gate:
        state = "READY"
        if adjusted_hint != "BẺ":
            adjusted_hint = "BẺ"
            adjusted_note = "AI điều phối: đã tới nhịp bẻ an toàn."
    elif flip_index >= gate - 8:
        state = "CAUTION"
        if adjusted_hint == "BẺ" and trap_score < 72 and turn_score < 0.18:
            adjusted_hint = "TRUNG_LẬP"
            adjusted_note = "AI điều phối: chưa đủ chắc, giảm nguy cơ bẻ sớm."
    else:
        state = "HOLD"
        if adjusted_hint == "BẺ" and bias < -0.10:
            adjusted_hint = "THEO"
            adjusted_note = "AI điều phối: đang có xu hướng bẻ sớm, chuyển sang theo."

    d["timing_last_gate"] = round(gate, 1)
    d["timing_last_index"] = round(flip_index, 1)
    d["timing_last_state"] = state

    return {
        "timing_bias": round(bias, 3),
        "timing_gate": round(gate, 1),
        "timing_index": round(flip_index, 1),
        "timing_state": state,
        "decision_hint": adjusted_hint,
        "decision_note": adjusted_note,
        "force_flip": state == "FORCE_FLIP",
        "ready": state == "READY",
...
def ai_level2_decision(d: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def pack(
        ...
        base_label = _root_label_from_context(root_label, last_label if _is_valid_label(last_label) else recent_best)
        final_label = base_label

        if hint == "BẺ":
            flipped = opposite_label(base_label)
            if _is_valid_label(flipped):
                final_label = flipped
        elif hint == "THEO":
            final_label = base_label
        ...
        return {
            ...
            "timing_state": "ON_TIME" if hint != "TRUNG_LẬP" else "WAIT",
            "timing_note": note,
            "source": source,
            "cau_type": cau_type,
            "white_type": white_type,
    ...
    if table_verdict == "LẶP GIẢ":
        flip_ready = (
            ...
            and (
                trap_is_lure
                or shift_score >= 0.18
                or early_shift_score >= 0.10
                or turn_score >= 0.10
                or float(report.get("white_score", 0.0)) >= 50.0
            )
        )

        if flip_ready and _timing_allows_flip(report):
            base = last_label if _is_valid_label(last_label) else recent_best

            conf = 76 if trap_score < 75 else 84
            if turn_score >= 0.16 or early_shift_score >= 0.14:
                conf += 4

            meta = pack(
                base,
                conf,
                "AI bảng thống kê chốt: lặp giả, đủ nhịp thì bẻ.",
                "CẦU LẶP GIẢ",
                "BẺ",
            )
            meta["decision_note"] = "Lặp giả đủ mạnh nên ưu tiên bẻ sớm."
            return meta

        root = _root_label_from_context(recent_best, last_label)
        meta = pack(
            root,
            60,
            "Lặp giả nhưng chưa đủ nhịp bẻ, tạm giữ theo đuôi gần nhất.",
            "CẦU LẶP GIẢ",
            "TRUNG_LẬP",
        )
        meta["decision_note"] = "Có dấu hiệu lặp giả nhưng chưa đủ chín để bẻ."
        return meta

    if table_verdict == "LẶP THẬT":
        cycle_pred, cycle_conf = predict_cycle_next(labels, cycle_len)
        if cycle_pred in (LOW_LABEL, HIGH_LABEL) and cycle_conf >= 58:
            conf = 80 if cycle_conf >= 70 else 72 if cycle_conf >= 55 else 66
            meta = pack(
                cycle_pred,
                conf,
                "AI bảng thống kê chốt: lặp thật, theo chu kỳ.",
                "CẦU LẶP THẬT",
                "THEO",
            )
            meta["decision_note"] = "Lặp thật đủ rõ, ưu tiên theo chu kỳ."
            return meta

    if trap_is_lure and trap_score >= 60 and _timing_allows_flip(report):
        root = _root_label_from_context(
            recent_best if recent_gap >= 0.04 else last_label,
            last_label,
        )
        meta = pack(
            root,
            78 if trap_score < 75 else 84,
            "Lặp giả: mẫu cũ đang dụ đảo, nhưng đã tới nhịp bẻ.",
            "CẦU LẶP GIẢ",
            "BẺ",
        )
        meta["decision_note"] = "Phát hiện lặp giả và có tín hiệu đổi pha."
        return meta

    if cau_type == "CẦU XEN KẼ":
        if last_label in (LOW_LABEL, HIGH_LABEL):
            meta = pack(
                opposite_label(last_label),
                82,
                "Cầu xen kẽ: bẻ theo nhịp đảo.",
                "CẦU XEN KẼ",
                "BẺ",
            )
    ...
    if cau_type in ("CẦU CHUYỂN PHA", "CẦU ĐUÔI MỚI"):
        ...
        if short_best in (LOW_LABEL, HIGH_LABEL):
            ...
            meta = pack(
                short_best,
                conf,
                "Cầu chuyển pha: ưu tiên đuôi gần nhất.",
                cau_type,
                "THEO",
            )
    ...
    if cau_type == "CẦU TRẮNG KHÁNG ÁM":
        meta = pack(
            ...
            "Cầu trắng kháng ám: ưu tiên nhịp mới sạch.",
            cau_type,
            "THEO",
        )
    ...
    if cau_type == "CẦU TRẮNG BỊ ÁM":
        meta = pack(
            ...
            "Cầu trắng bị ám: đọc đuôi mới nhưng thận trọng.",
            cau_type,
            "THEO",
        )
    ...
    if cau_type == "CẦU RUNG":
        ...
        if last_label in (LOW_LABEL, HIGH_LABEL):
            meta = pack(
                opposite_label(last_label),
                62,
                "Cầu rung: ưu tiên đảo nhịp ngắn hạn.",
                cau_type,
                "BẺ",
            )
    ...
    if cau_type == "CẦU HỖN LOẠN":
        if recent_gap >= 0.08:
            meta = pack(
                recent_best,
                60,
                "Cầu hỗn loạn: chọn tín hiệu gần hơn.",
                cau_type,
                "THEO",
            )
        ...
        if last_label in (LOW_LABEL, HIGH_LABEL):
            meta = pack(
                last_label,
                56,
                "Cầu hỗn loạn: bám nhịp mới nhất.",
                cau_type,
                "TRUNG_LẬP",
            )
    ...
    if cau_type == "CẦU ỔN ĐỊNH":
        if last_label in (LOW_LABEL, HIGH_LABEL) and streak >= 3:
            meta = pack(
                last_label,
                75,
                "Cầu ổn định: bám nhịp đang chạy.",
                cau_type,
                "THEO",
            )
    ...
    if recent_best in (LOW_LABEL, HIGH_LABEL):
        meta = pack(
            recent_best,
            60,
            "Cầu chưa rõ: ưu tiên đuôi gần nhất.",
            "CHƯA RÕ CẦU",
            "TRUNG_LẬP",
        )
...
def final_decision(report: Dict[str, Any], d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if base_label not in (LOW_LABEL, HIGH_LABEL):
        base_label = max(smoothed, key=smoothed.get)

    action_hint = meta.get("decision_hint", "TRUNG_LẬP")

    final_label = meta.get("final_label")
    if final_label not in (LOW_LABEL, HIGH_LABEL):
        final_label = base_label
    ...
    timing = timing_ai_governor(d, report, meta)
    meta["timing_ai"] = timing
    if timing.get("decision_hint") in ("BẺ", "THEO", "TRUNG_LẬP"):
        action_hint = timing["decision_hint"]
        meta["action_hint"] = action_hint
        meta["decision_hint"] = action_hint
        if timing.get("decision_note"):
            meta["decision_note"] = timing["decision_note"]
        if action_hint == "BẺ":
            final_label = opposite_label(base_label) or final_label
    ...
    if action_hint == "TRUNG_LẬP":
        conf = min(conf, max(32, int(conf * 0.90)))
    elif action_hint == "BẺ":
        conf = min(100, conf + 3)
    elif action_hint == "THEO":
        conf = min(100, conf + 1)

    # Tự hạ/đẩy độ tự tin theo tình trạng timing để giảm bẻ sớm hoặc bẻ trễ.
    if timing.get("timing_state") == "FORCE_FLIP":
        conf = min(100, conf + 4)
    elif timing.get("timing_state") == "CAUTION":
        conf = max(0, conf - 4)
    elif timing.get("timing_state") == "HOLD":
        conf = max(0, conf - 2)
...
def build_stats_message(report: Dict[str, Any], d: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"║ Dự đoán : {d.get('last_prediction_result', 'CHƯA RÕ')}\n"
        f"║ Bẻ/Theo : {d.get('last_action_result', 'CHƯA RÕ')}\n"
        f"║ Sai kiểu : {d.get('last_timing_fault', 'NONE')}\n"
        f"║ Hit/Miss : {d.get('prediction_hits', 0)}/{d.get('prediction_misses', 0)}\n"
        f"║ Bẻ đúng  : {d.get('flip_hits', 0)}/{d.get('flip_misses', 0)}\n"
        f"║ Theo đúng: {d.get('follow_hits', 0)}/{d.get('follow_misses', 0)}\n"
        "╠════════════════════════════╣\n"
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    async with STATE_LOCK:
        ...
        if nums:
            for n in nums:
                label = map_value(n)
                update_timing_feedback(d, label)
                d["values"].append(n)
...
async def debug_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            ...
            f"pred_res: {d.get('last_prediction_result', 'CHƯA RÕ')}\n"
            f"action_res: {d.get('last_action_result', 'CHƯA RÕ')}\n"
            f"timing_fault: {d.get('last_timing_fault', 'NONE')}\n"
            f"pred_hit/miss: {d.get('prediction_hits', 0)}/{d.get('prediction_misses', 0)}\n"
            f"flip_hit/miss: {d.get('flip_hits', 0)}/{d.get('flip_misses', 0)}\n"
            f"follow_hit/miss: {d.get('follow_hits', 0)}/{d.get('follow_misses', 0)}"
        )
    except Exception as e:
