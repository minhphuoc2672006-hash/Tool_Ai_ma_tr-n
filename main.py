...
def ai_self_heal(d):
    ...
    # lệch
    if len(h)>=8 and len(set(h[-6:]))==1:
        d["markov3"].clear()
        d["markov4"].clear()
        actions.append("⚠️ Reset lệch cầu")
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== SELF HEAL =====
    actions = ai_self_heal(d)
    if actions:
        await update.message.reply_text("🧠 AI Self-Heal:\n" + "\n".join(actions))
