...
ADMIN_USER_ID = int(os.getenv("ADMIN_USER_ID", "0"))  # đặt Telegram user ID của bạn vào biến môi trường
...
def detect_complex_cycle(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    # Chu kỳ 2 khối lặp (ví dụ 2-1-2-1 hoặc 3-2-3-2)
    if len(runs) >= 4:
    ...
    # Chu kỳ 3 khối lặp (ví dụ 1-2-1-1-2-1)
    if len(runs) >= 6:
...
def detect_false_trap(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    # Dạng A A A B A A hoặc B B B A B B thường là bẫy / nhử cầu
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str]) -> Dict[str, Any]:
    ...
    # Nếu phe bị lệch hoặc không đủ đồng thuận thì không chốt
