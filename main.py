...
# NHẬN DIỆN + DỰ ĐOÁN
# =========================
def classify_pattern_pro(seq: List[str]) -> Tuple[str, Optional[str], int, bool]:
    if len(seq) < 5:
    ...
    if soft_streak and soft_streak >= 3:
        return (f"Cầu bệt vừa {last_val} x{soft_streak}", last_val, 80, True)

    if streak_len >= 5:
        return (f"Cầu bệt {last_val} x{streak_len}", last_val, 90, True)
    ...
    if mixed:
        next_hint = last_val if last_val in {"T", "X"} else None
        return (mixed, next_hint, 63, True)
...
def save_current_prediction_if_any(seq: List[str]) -> None:
    ...
    pattern_label, next_hint, confidence, recognized = classify_pattern_pro(seq)
    if recognized and next_hint in {"T", "X"}:
        save_prediction(
