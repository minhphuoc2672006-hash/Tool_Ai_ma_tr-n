...
def run_strategy(name, history):
    ...
    base = name.split("_")[0]
    if base == "FOLLOW" and len(tx)>=1:
        return tx[-1]
    if base == "OPPOSITE" and len(tx)>=1:
        return "Xỉu" if tx[-1]=="Tài" else "Tài"
    if base == "TREND" and len(tx)>=4:
        return "Tài" if tx[-4:].count("Tài") > tx[-4:].count("Xỉu") else "Xỉu"
    if base == "STREAK" and len(tx)>=2:
        if tx[-1]==tx[-2]: return tx[-1]
    if base == "ANTI_STREAK" and len(tx)>=2:
        if tx[-1]==tx[-2]: return "Xỉu" if tx[-1]=="Tài" else "Tài"
    if base == "WEIGHTED":
        c = Counter(tx)
        return "Tài" if c["Tài"] > c["Xỉu"] else "Xỉu"
    if base == "LAST2" and len(tx)>=2:
        return tx[-2]
    if base == "ALT" and len(tx)>=2:
        return "Xỉu" if tx[-1]==tx[-2] else tx[-1]
    if base == "BREAK" and len(tx)>=3:
        if tx[-1]==tx[-2]==tx[-3]:
            return "Xỉu" if tx[-1]=="Tài" else "Tài"
    if base == "CHAOS":
        return random.choice(tx)

    if "P" in name and len(tx)>=3:
        return "Tài" if tx[-3:].count("Tài") > tx[-3:].count("Xỉu") else "Xỉu"
    if "M" in name and len(tx)>=2:
        last = tx[-2:]
...
def evaluate_strategies(history):
    ...
    for strat in STRATEGIES:
        ...
        for i in range(1, len(history)):
            ...
            if pred == real:
                wins += 1
        scores[strat] = wins / (len(history)-1) * 100
    best = max(scores, key=lambda k: scores[k])
    return best, scores[best]

# ===== GẤP THÉP =====
def bet_calc(user):
    base = int(user["start"] * 0.05)  # cược cơ bản luôn dựa trên vốn ban đầu
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== CHỈ PHÂN TÍCH KHI LỊCH SỬ >=5 =====
    if len(user["history"]) >= 5:
        # đánh giá chiến thuật tốt nhất dựa trên toàn bộ lịch sử
        best_strategy, conf = evaluate_strategies(user["history"])
        user["strategy"] = best_strategy
        # dự đoán theo chiến thuật tốt nhất
        pred = run_strategy(best_strategy, user["history"])
    else:
        best_strategy = user["strategy"]
        conf = 0.0
        pred = None  # chưa dự đoán

    next_bet = bet_calc(user)

    # ===== CẬP NHẬT TIỀN THẮNG/THUA =====
    if user["last_pred"] is not None and pred is not None:
    ...
    # lưu dự đoán và cược cho lần tiếp theo
    user["last_pred"] = pred
    ...
    msg = (
        ...
        "━━━━━━━━━━━━━━\n"
        f"🔮 Dự đoán lần tới: {pred if pred else 'Chưa đủ dữ liệu'}\n"
        f"📊 Xác suất: {conf:.1f}%\n"
        "──────────────\n"
