...
MAX_INPUT_NUMS = int(os.getenv("MAX_INPUT_NUMS", "1000"))
...
def cap_confidence(value: Any, clear_signal: bool = False) -> int:
    try:
        conf = int(round(float(value)))
    except Exception:
        conf = 0
    low = MIN_FINAL_CONFIDENCE if clear_signal else 50
    high = 90 if clear_signal else 65
    return max(low, min(high, conf))
...
def new_state() -> Dict[str, Any]:
    return {
        ...
        "max_wrong_streak": 0,
        "model_accuracy": {"pattern": 50, "markov": 50, "chart": 50, "structure": 50, "dice": 50},
        "last_note": "",
...
def repair_state(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(d.get("model_accuracy"), dict):
        d["model_accuracy"] = {"pattern": 50, "markov": 50, "chart": 50, "structure": 50, "dice": 50}
    defaults = new_state()
...
def parse_input(text: str) -> Tuple[List[int], List[List[int]]]:
    ...
    # Cắt bớt phần mô tả nếu có dạng "abc: dữ liệu"
    if ":" in text:
        left, right = text.split(":", 1)
        if any(ch.isdigit() for ch in right):
            text = right.strip()

    # Tách theo khoảng trắng / xuống dòng / dấu phẩy / chấm phẩy
    ...
    for token in raw_tokens:
        ...
        # Dạng 234 => xí ngầu 2-3-4
        if re.fullmatch(r"[1-6]{3}", token):
            flush_buffer()
            faces = [int(ch) for ch in token]
            dice_rolls.append(faces)
            totals.append(sum(faces))
            continue

        # Dạng 2-3-4, 12-13-13, 2/3/4, 2|3|4
        if re.fullmatch(r"\d+(?:[-/|]\d+)+", token):
            parts = [p for p in re.split(r"[-/|]", token) if p]
            if len(parts) == 3 and all(len(p) == 1 and p.isdigit() and 1 <= int(p) <= 6 for p in parts):
                flush_buffer()
                faces = [int(p) for p in parts]
                dice_rolls.append(faces)
                totals.append(sum(faces))
            else:
                flush_buffer()
        ...
        # Token không phải số: chốt buffer đang chờ
        flush_buffer()
...
def detect_transition_phase(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if diff >= 0.45:
        ...
        return {"name": "CHUYỂN PHA", "detail": f"Đoạn gần nhất nghiêng mạnh về {winner}", "score": 88, "hint": winner}
...
def detect_cycle_repeat(labels: List[str], max_window: int = 12) -> Optional[Dict[str, Any]]:
    ...
    for size in range(2, upper + 1):
        ...
        if reps >= 2:
            ...
            cand = {
                "name": label,
                "detail": f"Nhịp run-length {'-'.join(map(str, motif))} lặp {reps} lần",
                "score": score,
                "hint": tail_runs[-1][0],
                "pattern_counts": motif,
                "signature": "-".join(map(str, motif)),
            }
            if best is None or cand["score"] > best["score"]:
                best = cand
    return best
...
def detect_all_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    if last in (LOW_LABEL, HIGH_LABEL) and streak >= 3:
        patterns.append({"name": "BỆT", "detail": f"{last} x{streak}", "score": min(68 + streak * 6, 95), "hint": last, "signature": f"BET-{last}-{streak}"})

    alt, alt_ratio = alternating_tail(labels, 6)
    if alt and alt_ratio >= 0.80 and len(labels) >= 6:
        patterns.append({"name": "XEN KẼ", "detail": "Chuỗi đổi liên tục", "score": 88, "hint": opposite_label(labels[-1]), "signature": "ALT-6"})
...
def deep_structure_analysis(labels: List[str]) -> Dict[str, Any]:
    ...
    if not labels:
        return {
            ...
            "chaos_score": 0,
            "stability_score": 50,
            "structure_hint": "CHƯA ĐỦ DỮ LIỆU",
            "repeat_note": "Chưa có dữ liệu",
        }
...
def pattern_signature(pattern: Dict[str, Any]) -> str:
    ...
    if sig:
        return str(sig)
    counts = pattern.get("pattern_counts")
    name = str(pattern.get("name", ""))
    hint = str(pattern.get("hint", ""))
    if isinstance(counts, list) and counts:
        return f"{name}|{hint}|{'-'.join(map(str, counts))}"
    return f"{name}|{hint}"
...
def prediction_gate(labels: List[str], report: Dict[str, Any], state: Optional[Dict[str, Any]] = None) -> Tuple[bool, str]:
    ...
    if state is not None:
        ...
        state["last_gate_status"] = "CẦU RÕ" if clear_signal else "THEO DÕI"
        state["last_gate_reason"] = reason
    else:
        clear_signal, reason = False, "Chưa kiểm tra"
    if not patterns:
        return False, "Chưa thấy cầu rõ"
    top = patterns[0]
    top_score = int(top.get("score", 0))
    chaos = int(report.get("chaos_score", 0))
    structure_hint = str(report.get("structure_hint", ""))
    if top_score >= 70 and chaos <= 72 and structure_hint != "CẦU LOẠN" and total >= MIN_PREDICTION_DATA:
        return True, f"Cầu rõ: {top.get('name', '-')}"
    if top_score >= 58 and chaos <= 82:
        return False, f"Đang theo dõi: {top.get('name', '-')} ({top_score}%)"
...
def predict_structure(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        name = top.get("name", "")
        strong_same = {
            "BỆT", "BỆT SỚM", "LẶP MẪU 1", "LẶP KHỐI 1",
            "XEN KẼ SỚM", "XEN KẼ", "XEN KẼ SÂU",
            "1-1", "2-2", "SÓNG ĐỀU", "CẦU PHỨC HỢP",
            "CHU KỲ MỞ RỘNG",
        }
...
def update_model_accuracy(state: Dict[str, Any], predictions: Dict[str, Dict[str, Any]], actual: str) -> None:
    state.setdefault("model_accuracy", {"pattern": 50, "markov": 50, "chart": 50, "structure": 50, "dice": 50})
...
def filter_predictions_for_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> Tuple[Dict[str, Dict[str, Any]], List[str]]:
    ...
    for name, pred in predictions.items():
        ...
        if keep:
            ...
            notes.append(f"loại:{name}({int(conf)}%)")
    if not filtered and predictions:
        ...
        notes.append(f"dự phòng:{best_name}")
    notes.append(f"giữ={len(filtered)}/{len(predictions)}")
...
def meta_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any], clear_signal: bool) -> Dict[str, Any]:
    ...
    if not vote:
        ...
        return {
            "model": "dự phòng",
            "final_label": fallback.get("label", LOW_LABEL),
            "confidence": max(50, int(fallback.get("confidence", 50)) - 2),
            "scores": {fallback.get("label", LOW_LABEL): float(fallback.get("confidence", 50))},
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str], dice_rolls_full: Optional[List[List[int]]] = None) -> Dict[str, Any]:
    ...
    state["last_structure"] = report.get("short_structure", report.get("structure", "CHƯA ĐỦ DỮ LIỆU"))
    ...
    if clear_signal:
        ...
        state["last_analysis_trace"] = (
            f"mô_hình={pretty_model_name(meta.get('model', 'none'))} | "
            f"đồng_thuận={meta.get('agreement', 0)}/{len(predictions)} | "
            f"tỷ_lệ={meta.get('top_ratio', 0):.2f} | "
            f"điểm={meta.get('scores', {})} | "
            f"cửa_gác={state.get('last_gate_status', 'CHỜ')}: {state.get('last_gate_reason', '-')}"
        )
        if meta["confidence"] < MIN_FINAL_CONFIDENCE:
            state["last_prediction_label"] = None
            state["last_prediction_conf"] = 0
            state["last_prediction_result"] = "THEO DÕI"
            state["last_note"] = f"{reason} | chưa đủ chắc để chốt"
        else:
            ...
            state["last_note"] = f"Mô hình: {pretty_model_name(meta['model'])} | Đồng thuận: {meta['agreement']}/{len(predictions)}"
    else:
        meta = {
            "model": "watch",
            "final_label": "CHƯA RÕ",
            ...
            "filter_notes": ["theo dõi"],
            "filtered_count": len(predictions),
            "raw_count": len(predictions),
        }
        state["last_analysis_trace"] = f"theo dõi | cửa_gác={reason}"
        state["last_prediction_label"] = None
        state["last_prediction_conf"] = 0
        state["last_prediction_result"] = "THEO DÕI"
...
def build_chart_summary(report: Dict[str, Any], adv: Dict[str, Any], state: Optional[Dict[str, Any]] = None) -> str:
    return (
        ...
        f"Cửa sổ: {state.get('last_analysis_window', '-') if state else '-'}\n"
        ...
        f"Đà: {adv.get('momentum',0.0):.2f} | Xu hướng: {adv.get('trend_label','TRUNG TÍNH')} ({adv.get('trend',0.0):.2f})\n"
        f"Mượt: {adv.get('smoothness',0.0):.2f} | Đảo chiều: {adv.get('reversal',0.0):.1f}%\n"
        f"Entropy: {report.get('entropy',0.0):.2f} | Độ biến động: {report.get('volatility',0.0):.2f}\n"
        f"Chu kỳ: {report.get('cycle_signature', '-')}\n"
        f"Lặp: {report.get('repeat_signature', '-')} x{report.get('repeat_count', 0)}\n"
        f"Loạn/Ổn định: {report.get('chaos_score', 0)}/{report.get('stability_score', 50)}"
    )
...
def build_dice_chart_summary(summary: Dict[str, Any]) -> str:
    ...
    return (
        f"Số lượt: {summary.get('total_rolls', 0)} | "
        f"Tổng trung bình: {summary.get('avg_total', 0.0):.2f}\n"
        f"TB viên 1/2/3: {avg_faces[0]:.2f} / {avg_faces[1]:.2f} / {avg_faces[2]:.2f}\n"
        f"Độ lệch chuẩn viên 1/2/3: {std_faces[0]:.2f} / {std_faces[1]:.2f} / {std_faces[2]:.2f}\n"
        ...
        f"Mặt xuất hiện nhiều: {hot_text}\n"
        f"Mẫu: {summary.get('pattern', '-')}\n"
        f"Chi tiết: {summary.get('detail', '-')}"
    )
...
def build_dice_chart_image(dice_rolls: List[List[int]], summary: Dict[str, Any]) -> Optional[BytesIO]:
    ...
    ax.text(0.98, 0.02, "Mỗi đường là một viên riêng", transform=ax.transAxes, va="bottom", ha="right", fontsize=9, color="white", bbox=dict(boxstyle="round,pad=0.35", facecolor="#1f2937", alpha=0.9, edgecolor="#6b7280"))
    fig.tight_layout()
    buf = BytesIO()
    fig.savefig(buf, format="png", bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close(fig)
...
def build_final_message(meta: Dict[str, Any], state: Dict[str, Any]) -> str:
    return (
        ...
        f"🏆 Thắng / Thua: {state.get('prediction_hits', 0)} / {state.get('prediction_misses', 0)}\n"
        f"📈 WR tổng: {overall_winrate(state):.1f}% | WR gần 20: {recent_winrate(state, 20):.1f}%\n"
        ...
        f"🎲 Xí ngầu sâu: {state.get('dice_deep_summary') or '-'}\n"
...
def build_trace_message(state: Dict[str, Any]) -> str:
    ...
    if isinstance(model_preds, dict) and model_preds:
        ...
        model_text = "-"
    return (
        "🧾 NHẬT KÝ PHÂN TÍCH GẦN NHẤT\n"
        f"• Cửa sổ: {state.get('last_analysis_window', '-')}\n"
        f"• Mô hình AI: {'BẬT' if state.get('ai_enabled', True) else 'TẮT'}\n"
        f"• Mô hình HASH: {'BẬT' if state.get('hash_enabled', True) else 'TẮT'}\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/ai - phân tích cầu\n"
        "/next - giống /ai\n"
        "/why - xem lý do chốt/theo dõi gần nhất\n"
        "/aimodel - bật/tắt hiển thị mô hình AI\n"
        "/hashmodel - bật/tắt mô hình hash/MD5\n"
...
async def aimodel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users[chat_id] = state
    trim_cache()
    await update.message.reply_text(f"🤖 Mô hình AI: {'BẬT' if state['ai_enabled'] else 'TẮT'}")
...
async def hashmodel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users[chat_id] = state
    trim_cache()
    await update.message.reply_text(f"🔷 Mô hình HASH: {'BẬT' if state['hash_enabled'] else 'TẮT'}")
