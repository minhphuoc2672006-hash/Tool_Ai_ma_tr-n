...
def markov_dynamic_predict(history):
    ...
    # dùng toàn bộ history
    for k in range(n - 1, 0, -1):
        key = tuple(history[-k:])

        # duyệt toàn bộ lịch sử để tìm match
        for i in range(n - k):
            if tuple(history[i:i+k]) == key:
                nxt = history[i+k]

                # trọng số cực mạnh theo độ dài + vị trí mới
                recency_bonus = (i / n) * 5
                weight = (k * 10) + recency_bonus
...
def final_ai(data):
    ...
    all_preds += markov_dynamic_predict(history)
