...
def forecast_next(chat_id: int, report: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    scores = {
        LOW_LABEL: (
            recent_probs[LOW_LABEL] * 0.40
            + global_probs[LOW_LABEL] * 0.20
            + markov_probs[LOW_LABEL] * 0.40
        ),
        HIGH_LABEL: (
            recent_probs[HIGH_LABEL] * 0.40
            + global_probs[HIGH_LABEL] * 0.20
            + markov_probs[HIGH_LABEL] * 0.40
        ),
    }
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        report_now = analyze_sequence(await load_user(chat_id))
