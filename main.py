...
def filter_predictions_for_decision(
    ...
    """
    Lọc trước, chốt sau:
    - Giữ những hệ đủ mạnh.
    - Giữ những hệ yếu hơn nhưng đang cùng hướng với chart/structure.
    - Loại các hệ nhiễu để không kéo kết quả chốt.
    """
    ...
    for name, pred in predictions.items():
        ...
        if name == "chart":
            if conf >= 72 and float(adv.get("smoothness", 0.0)) >= 0.48:
                keep = True
                reason = "chart mạnh"
            elif conf >= thresholds[name]:
                keep = True
                reason = "chart đạt ngưỡng"
        elif name == "dice":
            avg_total = float(report.get("avg_total", 0.0)) if isinstance(report, dict) else 0.0
            if conf >= 64 and (avg_total >= 11.0 or avg_total <= 10.0 or max(adv.get("recent_volatility", [0.0, 0.0, 0.0])) >= 0.9):
                keep = True
                reason = "dice có lệch rõ"
            elif conf >= thresholds[name]:
                keep = True
                reason = "dice đạt ngưỡng"
        else:
            if conf >= thresholds.get(name, 60):
                keep = True
                reason = "đạt ngưỡng"

        if not keep and label in (chart_label, structure_label):
            if float(chart.get("confidence", 0)) >= 68 or float(structure.get("confidence", 0)) >= 68:
                keep = True
                reason = "cùng hướng với chart/structure"

        if not keep and chart_label in (LOW_LABEL, HIGH_LABEL) and label == chart_label:
            if name in {"dice", "pattern", "markov"} and float(chart.get("confidence", 0)) >= 75:
                keep = True
                reason = "chart kéo cùng hướng"
...
def meta_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not vote:
        ...
        return {
            ...
            "confidence": max(50, int(fallback.get("confidence", 50)) - 3),
    ...
    confidence = int(48 + top_ratio * 40 + (agreement - 1) * 6 + (strongest_conf - 50) * 0.20)
    if filtered_predictions.get("chart", {}).get("label") == best_label and float(filtered_predictions.get("chart", {}).get("confidence", 0)) >= 70:
        confidence += 6
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str], dice_rolls_full: Optional[List[List[int]]] = None) -> Dict[str, Any]:
    ...
    if meta["top_ratio"] < MIN_CONSENSUS_RATIO or meta["agreement"] < 2 or meta["confidence"] < MIN_FINAL_CONFIDENCE:
        ...
        return {
            ...
            "meta": meta,
            "allowed": False,
            "reason": f"Thiếu đồng thuận ({meta['agreement']}/{len(predictions)})",
            "window": window,
            "labels_used": labels,
            "dice_used": dice_rolls,
        }
