...
def clean_noise(d):
    ...
    if zigzag and d["lose"] >= 4 and len(set(h)) == 2:
        if h[-1] != h[-2]:
...
def detect_trap(h_raw):
    ...
    zigzag = all(last6[i] != last6[i-1] for i in range(1, 6))
    if zigzag and last6.count("Tài") >= 3 and last6.count("Xỉu") >= 3:
        res.append(("CẦU ẢO", None, 0))

    if len(h) >= 6 and all(h[i] != h[i-1] for i in range(len(h)-6, len(h)-2)) and len(set(h[-2:])) == 1:
...
def final_ai(d):
    ...
    if d["lose"] >= 5:
        smart_reset(d)
        return None, 0, [], [], "♻️ Reset AI do thua sâu"
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("""🔄 RESET HOÀN TOÀN
...
""")
    ...
    await msg.edit_text(f"""╔══ 🤖 AI CHIẾN THỰC ══╗
{status}
