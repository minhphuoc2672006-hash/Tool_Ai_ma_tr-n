...
def detect_all_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    return _dedupe_patterns(patterns)[:15]
...
def web_scan_history(labels: List[str], dice_rolls: Optional[List[List[int]]] = None) -> Optional[Dict[str, Any]]:
    ...
    windows = (6, 8, 10, 12, 16, 20, 24, 30, 36, 48)
    ...
    for w in windows:
        ...
        for item in (
            ...
            if item:
                seg_candidates.append(item)

        if not seg_candidates:
            continue

        best = sorted(seg_candidates, key=lambda x: x.get("score", 0), reverse=True)[0]
        hint = best.get("hint")
        ...
        support = 0
        for probe in (6, 12, 18, 24):
            if len(tail) >= probe:
                sub = tail[-probe:]
                if sub[-1] == hint:
                    support += 1
                elif sub.count(hint) >= max(2, int(probe * 0.55)):
                    support += 1

        score = min(95, int(best.get("score", 0)) + support * 3 + (2 if w <= 12 else 0))
        candidates.append({
    ...
    best = max(candidates, key=lambda x: x.get("score", 0), default=None)

    if best and dice_hint in (LOW_LABEL, HIGH_LABEL) and best.get("hint") == dice_hint:
        best["score"] = min(95, int(best["score"]) + 4)
