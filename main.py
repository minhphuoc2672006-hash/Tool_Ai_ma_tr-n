...
def _timing_allows_flip(report: Dict[str, Any]) -> bool:
    ...
    return (
        turn_score >= 0.18
        or early_shift_score >= 0.15
        or shift_score >= WHITE_SHIFT_THRESHOLD
        or (white_score >= WHITE_MIN_SCORE and ghost_score >= GHOST_SOFT_CLEAN)
    )
...
def ai_level2_decision(d: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if table_verdict == "LẶP GIẢ":
        if trap_score >= 70 and _timing_allows_flip(report):
            root = _root_label_from_context(
                recent_best if recent_gap >= 0.04 else last_label,
                last_label,
            )
            conf = 80 if trap_score >= 80 else 74
            if recent_gap >= 0.08:
                conf += 4
            ...
            meta = pack(
                root,
                conf,
                "AI bảng thống kê chốt: lặp giả, nhưng đã tới nhịp bẻ.",
                "CẦU LẶP GIẢ",
                "BẺ",
            )
            meta["decision_note"] = "Lặp giả đã đủ tín hiệu đổi pha, mới cho bẻ."
            return meta
        ...
        meta = pack(
            root,
            60,
            "Lặp giả nhưng chưa tới nhịp bẻ, tạm giữ theo đuôi gần nhất.",
            "CẦU LẶP GIẢ",
            "TRUNG_LẬP",
        )
