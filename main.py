...
def final_decision(
    ...
    reasons = [
        f"AI nhịp mới: {rhythm_ai.get('status', '-')}",
        f"AI chống ám: {ghost_ai.get('status', '-')}",
        f"RECHECK: {recheck_status}",
        f"Giám sát: {monitor.get('severity', '-')}",
    ]
...
def build_final_message(meta: Dict[str, Any]) -> str:
    ...
    other = meta.get("other_label", "-")
    conf = meta.get("confidence", 0)
    recheck_status = meta.get("recheck_status", "DISABLED")
    recheck_conf = meta.get("recheck_confidence", 0)
    return (
        f"DỰ ĐOÁN: {label}\n"
        f"DỰ PHÒNG: {other}\n"
        f"RECHECK: {recheck_status} | {recheck_conf}%\n"
...
async def delayed_analysis_job(chat_id: int, version: int, loading_msg) -> None:
    try:
        ...
        meta = final_decision(report, rhythm_ai, ghost_ai, recheck, monitor)
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        async with STATE_LOCK:
            ...
            meta = final_decision(report_now, rhythm_ai, ghost_ai, recheck, monitor)
