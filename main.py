...
def format_history(labels: List[str], tail: int = 24) -> str:
    """Render a compact history strip for the stats board."""
    if not labels:
        return "(trống)"
...
def format_pattern_lines(patterns: List[Tuple[str, float]], top_n: int = 3) -> str:
    """Render the strongest learned patterns in a compact text line."""
    if not patterns:
        return "Không có"
    items = []
...
def jensen_shannon_divergence(p: Dict[str, float], q: Dict[str, float]) -> float:
    ...
    m = {LOW_LABEL: (p.get(LOW_LABEL, 0.5) + q.get(LOW_LABEL, 0.5)) / 2.0, HIGH_LABEL: (p.get(HIGH_LABEL, 0.5) + q.get(HIGH_LABEL, 0.5)) / 2.0}
...
def final_decision(report: Dict[str, Any], rhythm_ai: Dict[str, Any], ghost_ai: Dict[str, Any], recheck: Dict[str, Any], monitor: Dict[str, Any], d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Không ưu tiên bên nào chỉ vì bên đó nhiều hơn.
    # Chốt chỉ bám cầu, nhịp gần, recheck và độ ổn định của bảng thống kê.
    scores = {
    ...
    best_label = HIGH_LABEL if probs[HIGH_LABEL] > probs[LOW_LABEL] else LOW_LABEL
    if diff < 0.028:
        last_label = report.get("last_label")
        streak = int(report.get("streak", 0))
        if report.get("cycle_len") and cycle_consistency < 60:
            best_label = HIGH_LABEL if report.get("cycle_score", 0.0) >= 70 else LOW_LABEL
        elif last_label in (LOW_LABEL, HIGH_LABEL) and streak >= 3:
            best_label = HIGH_LABEL if last_label == LOW_LABEL else LOW_LABEL
        elif recheck_status == "RECHECK_SHIFT":
            best_label = HIGH_LABEL if report.get("shift_score", 0.0) < 0.20 else LOW_LABEL
        elif report.get("volatility", 0.0) >= 0.45:
            best_label = HIGH_LABEL if report.get("entropy", 0.0) >= 0.75 else LOW_LABEL
        else:
            best_label = HIGH_LABEL if probs[HIGH_LABEL] >= 0.5 else LOW_LABEL

    other_label = HIGH_LABEL if best_label == LOW_LABEL else LOW_LABEL
    delta = abs(probs[LOW_LABEL] - probs[HIGH_LABEL])
    confidence = int(max(0, min(100, (rhythm_ai.get("confidence", 0) * 0.38) + (ghost_ai.get("confidence", 0) * 0.38) + (recheck_conf * 0.16) + delta * 28 + (5 if stability >= 70 else 0) - (8 if stability < 45 else 0))))
    if diff < 0.03:
        confidence = min(confidence, 68)
    if cycle_score >= 70 and cycle_len and cycle_consistency < 60:
        confidence = min(confidence, 67)

    return {"final_label": best_label, "other_label": other_label, "confidence": confidence, "final_probs": probs, "recheck_status": recheck_status, "recheck_confidence": recheck_conf, "source": "FINAL_LOCK"}
...
def build_loading_message(report: Dict[str, Any], rhythm_ai: Dict[str, Any], ghost_ai: Dict[str, Any], monitor: Dict[str, Any], recheck: Dict[str, Any]) -> str:
    return (
        "╔════════════════════════════╗\n"
        "║      ⏳ ĐANG PHÂN TÍCH      ║\n"
        "╠════════════════════════════╣\n"
...
def build_analysis_message(report: Dict[str, Any], rhythm_ai: Dict[str, Any], ghost_ai: Dict[str, Any], meta: Dict[str, Any], monitor: Dict[str, Any], recheck: Dict[str, Any]) -> str:
    return (
        "╔════════════════════════════╗\n"
        "║       🔍 PHÂN TÍCH         ║\n"
        "╠════════════════════════════╣\n"
...
def build_final_message(meta: Dict[str, Any]) -> str:
    return f"DỰ ĐOÁN: {meta.get('final_label', '-')}\nTỶ LỆ: {meta.get('confidence', 0)}%"
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if update.message:
            ...
            if cleanup.get("cleaned"):
                await update.message.reply_text(cleanup.get("message", "🧹 Bot vừa tự dọn dữ liệu cũ."))
            loading_msg = await update.message.reply_text(build_loading_message(report_now, rhythm_ai, ghost_ai, monitor, recheck))
            await start_delayed_analysis(context, chat_id, loading_msg)
    except Exception as e:
