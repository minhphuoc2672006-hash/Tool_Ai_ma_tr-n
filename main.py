...
def trim_state_memory(d: Dict[str, Any]) -> None:
    d["values"] = d.get("values", [])[-MAX_LABELS_STORE:]
    d["labels"] = d.get("labels", [])[-MAX_LABELS_STORE:]
    d["history"] = d.get("history", [])[-50:]
    d["health_log"] = d.get("health_log", [])[-MAX_HEALTH_LOG:]
...
def prune_chat_rows(conn: sqlite3.Connection, chat_id: int) -> None:
    row = conn.execute(
        "SELECT id FROM history WHERE chat_id = ? ORDER BY id DESC LIMIT 1 OFFSET ?",
        (chat_id, max(0, MAX_DB_HISTORY - 1)),
    ).fetchone()
    if row:
...
async def save_user_meta(chat_id: int, d: Dict[str, Any]) -> None:
    async with DB_LOCK:
        health_json = json.dumps(d.get("health_log", [])[-MAX_HEALTH_LOG:], ensure_ascii=False)
        with db_connect() as conn:
            conn.execute(
                """
...
def choose_balanced_label(
    ...
    if last_label in (LOW_LABEL, HIGH_LABEL) and streak >= 2:
        return HIGH_LABEL if last_label == LOW_LABEL else LOW_LABEL
    seed = (chat_id * 1315423911 + total * 2654435761 + low * 97 + high * 53) & 1
    return HIGH_LABEL if seed else LOW_LABEL
...
# WHITE / ANTI-GHOST
# =========================
def detect_white_cau(labels: List[str]) -> Dict[str, Any]:
    total = len(labels)
    ...
    freshness = max(0.0, min(1.0, 1.0 - (len(old) / max(1, total))))

    ghost_score = (
        shift_score * 55.0
        + old_bias * 20.0
        + max(0.0, recent_entropy - 0.55) * 15.0
        + recent_vol * 10.0
    )
    white_score = max(0.0, min(100.0, ghost_score))
...
def anti_ghost_cleanup(d: Dict[str, Any], report: Dict[str, Any]) -> None:
...
def master_controller(d: Dict[str, Any], white_info: Dict[str, Any]) -> Tuple[str, str]:
    labels = d.get("labels", [])
    if len(labels) < 10:
        return "WARMUP", "Đang học dữ liệu"

    white_type = white_info.get("white_type", "CHƯA RÕ CẦU TRẮNG")
    white_score = float(white_info.get("white_score", 0.0))
...
def analyze_sequence(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    white_info = detect_white_cau(labels)
    white_type = white_info.get("white_type", "CHƯA RÕ CẦU TRẮNG")
    white_detail = white_info.get("white_detail", "")
    white_score = float(white_info.get("white_score", 0.0))
    ghost_score = float(white_info.get("ghost_score", 0.0))
    shift_score = float(white_info.get("shift_score", 0.0))

    mode, note = master_controller(d, white_info)
    ...
    return {
        ...
        "ghost_score": ghost_score,
        "shift_score": shift_score,
        "recent_entropy": white_info.get("recent_entropy", 0.0),
        "recent_volatility": white_info.get("recent_volatility", 0.0),
        "old_bias": white_info.get("old_bias", 0.0),
        "recent_bias": white_info.get("recent_bias", 0.0),
        "recent_low_ratio": white_info.get("recent_low_ratio", 0.5),
        "old_low_ratio": white_info.get("old_low_ratio", 0.5),
        "freshness": white_info.get("freshness", 0.0),
        "mid_bias": white_info.get("mid_bias", 0.0),
...
def anti_ghost_weights(report: Dict[str, Any], labels: List[str]) -> Tuple[float, float, float, float]:
    ...
    if white_type == "CẦU TRẮNG KHÁNG ÁM":
        rec_w, mid_w, old_w, markov_w = 0.72, 0.18, 0.00, 0.10
    elif white_type == "CẦU TRẮNG BỊ ÁM":
        rec_w, mid_w, old_w, markov_w = 0.80, 0.14, 0.00, 0.06
    elif white_type == "CẦU CHUYỂN PHA":
        rec_w, mid_w, old_w, markov_w = 0.78, 0.14, 0.00, 0.08
    elif white_type == "CẦU TRẮNG":
        rec_w, mid_w, old_w, markov_w = 0.68, 0.20, 0.02, 0.10
...
def forecast_next(chat_id: int, report: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    cau_detail = report.get("cau_detail", "")
    white_type = report.get("white_type", "CHƯA RÕ CẦU TRẮNG")
    ...
    recent_probs = weighted_label_probs(recent_labels, window=min(RECENT_WINDOW, len(recent_labels)), decay=0.91)
    mid_probs = weighted_label_probs(mid_labels, window=min(MID_WINDOW, len(mid_labels)), decay=0.96)
    ...
    scores = {
        LOW_LABEL: (
            recent_probs[LOW_LABEL] * rec_w
            + mid_probs[LOW_LABEL] * mid_w
            + old_probs[LOW_LABEL] * old_w
            + markov_probs[LOW_LABEL] * markov_w
            + global_probs[LOW_LABEL] * 0.02
        ),
        HIGH_LABEL: (
            recent_probs[HIGH_LABEL] * rec_w
            + mid_probs[HIGH_LABEL] * mid_w
            + old_probs[HIGH_LABEL] * old_w
            + markov_probs[HIGH_LABEL] * markov_w
            + global_probs[HIGH_LABEL] * 0.02
        ),
    }
    ...
    if cau_type == "CẦU XEN KẼ" and last_label in (LOW_LABEL, HIGH_LABEL):
        ...
        scores[cycle_pred] *= 1.35
        scores[cycle_pred] += (cycle_pred_conf / 100.0) * 0.15
    ...
    if abs(probs[LOW_LABEL] - probs[HIGH_LABEL]) >= 0.02:
        best_label = max(probs, key=probs.get)
    else:
        best_label = choose_balanced_label(
            chat_id,
            total,
            report.get("low", 0),
            report.get("high", 0),
    ...
    if delta < 0.08:
        status = "BALANCED"
        message = "Hai phía khá cân bằng."
    else:
        status = "TRENDING"
        message = f"Xu hướng theo cầu nghiêng về {best_label}."
...
def meta_ai_judge(chat_id: int, report: Dict[str, Any], ai: Dict[str, Any]) -> Dict[str, Any]:
    probs = dict(ai.get("probs", {LOW_LABEL: 0.5, HIGH_LABEL: 0.5}))
    ...
    final_label = ai.get("best_label")
    final_probs = dict(probs)

    if white_type == "CẦU TRẮNG KHÁNG ÁM":
        final_label = ai.get("best_label")
        reasons.append("cầu trắng kháng ám")
    elif white_type == "CẦU TRẮNG BỊ ÁM":
        final_label = ai.get("best_label")
        reasons.append("cầu trắng bị ám")
    elif white_type == "CẦU CHUYỂN PHA":
        final_label = ai.get("best_label")
        reasons.append("cầu chuyển pha")
    elif cau_type == "CẦU XEN KẼ" and last_label in (LOW_LABEL, HIGH_LABEL):
        final_label = HIGH_LABEL if last_label == LOW_LABEL else LOW_LABEL
        reasons.append("cầu xen kẽ")
    elif cau_type == "CẦU BỆT" and last_label in (LOW_LABEL, HIGH_LABEL):
        final_label = HIGH_LABEL if last_label == LOW_LABEL else LOW_LABEL
        reasons.append("cầu bệt")
    elif cau_type == "CẦU LẶP":
        cycle_pred = ai.get("cycle_pred")
        if cycle_pred in (LOW_LABEL, HIGH_LABEL):
            final_label = cycle_pred
            reasons.append(f"cầu lặp {cycle_len}")
        else:
            final_label = ai.get("best_label")
            reasons.append("cầu lặp chưa khóa được nhánh")
    elif cau_type == "CẦU RUNG":
        if last_label in (LOW_LABEL, HIGH_LABEL):
            final_label = HIGH_LABEL if last_label == LOW_LABEL else LOW_LABEL
            reasons.append("cầu rung đổi nhịp")
    elif cau_type == "CẦU ỔN ĐỊNH":
        final_label = ai.get("best_label")
        reasons.append("cầu ổn định")
    else:
        final_label = ai.get("best_label")
        reasons.append("chưa khóa được cầu")

    if final_label == LOW_LABEL:
        final_probs[LOW_LABEL] += 0.08
        final_probs[HIGH_LABEL] -= 0.05
    elif final_label == HIGH_LABEL:
        final_probs[HIGH_LABEL] += 0.08
        final_probs[LOW_LABEL] -= 0.05

    if streak >= 6 and last_label in (LOW_LABEL, HIGH_LABEL):
        reasons.append(f"chuỗi {last_label} x{streak}")
        confidence += 4
    if alt and alt_ratio >= 0.90:
        reasons.append("xen kẽ mạnh")
        confidence += 3
    if cycle_score >= 70 and cycle_len:
        reasons.append(f"vòng lặp {cycle_len}")
        confidence += 4
    if white_type in ("CẦU TRẮNG KHÁNG ÁM", "CẦU TRẮNG BỊ ÁM", "CẦU CHUYỂN PHA"):
        reasons.append(f"trắng {white_type}")
        confidence += 3 if white_score >= 70 else 1
        if shift_score >= WHITE_SHIFT_THRESHOLD:
            reasons.append("ám cũ được xử lý")
            confidence += 2
    if freshness >= 0.65 and shift_score >= DRIFT_MIN_SHIFT:
        reasons.append("ưu tiên dữ liệu mới")
        confidence += 2
    if mode == "NOISY" or (vol >= 0.85 and ent >= 0.85):
        reasons.append("nhiễu cao")
        confidence -= 8
    if mode == "WARMUP":
    ...
    if final_label not in (LOW_LABEL, HIGH_LABEL):
        final_label = choose_balanced_label(chat_id, total, low, high, last_label, streak)

    return {
        "final_label": final_label,
        "confidence": confidence,
        "final_probs": final_probs,
        "reasons": reasons if reasons else ["tổng hợp tín hiệu hiện tại"],
        "cau_type": cau_type,
        "cau_detail": cau_detail,
        "white_type": white_type,
        "white_score": white_score,
...
def build_loading_message(report: Dict[str, Any]) -> str:
    return (
        "╔════════════════════════════╗\n"
        "║      ⏳ ĐANG PHÂN TÍCH      ║\n"
        "╠════════════════════════════╣\n"
        ...
        f"║ AI cấp 2 đang đọc bảng cầu...\n"
        "╚════════════════════════════╝"
    )


def build_analysis_message(report: Dict[str, Any], ai: Dict[str, Any], meta: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"║ Ghost   : {report.get('ghost_score', 0.0):.0f}\n"
        f"║ Ám cũ   : {report.get('shift_score', 0.0):.2f}\n"
        f"║ Sơ cấp  : {ai.get('status', '-')} | {ai.get('confidence', 0)}%\n"
        f"║ PX sơ cấp: {format_prob_inline(probs)}\n"
        f"║ Chốt cuối: {meta.get('final_label', '-')} | {meta.get('confidence', 0)}%\n"
        f"║ PX chốt : {format_prob_inline(final_probs)}\n"
        f"║ Lý do   : {'; '.join(meta.get('reasons', [])[:4])}\n"
        "╚════════════════════════════╝"
    )
...
def build_final_message(meta: Dict[str, Any]) -> str:
    ...
    return (
        f"DỰ ĐOÁN: {label}\n"
        f"TỶ LỆ: {conf}%  |  {LOW_LABEL} {final_probs.get(LOW_LABEL, 0.5) * 100:.1f}%  -  {HIGH_LABEL} {final_probs.get(HIGH_LABEL, 0.5) * 100:.1f}%"
    )
...
async def delayed_analysis_job(chat_id: int, version: int, loading_msg) -> None:
    try:
        ...
        ai = forecast_next(chat_id, report, d.get("labels", []))
        meta = meta_ai_judge(chat_id, report, ai)

        if analysis_versions.get(chat_id, 0) != version:
            return

        try:
            await loading_msg.edit_text(build_analysis_message(report, ai, meta))
        except Exception:
            pass
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        await update.message.reply_text(
            ...
            f"Bot đã tách 3 tầng dữ liệu: ngắn hạn / trung hạn / dài hạn.\n"
            f"Bot ưu tiên tín hiệu mới khi phát hiện cầu trắng hoặc đổi pha.\n"
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(build_stats_message(report, d))
        loading_msg = await update.message.reply_text(build_loading_message(report))
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        async with STATE_LOCK:
            ...
            anti_ghost_cleanup(d, report_now)
            trim_state_memory(d)
        ...
        if update.message:
            ...
            loading_msg = await update.message.reply_text(build_loading_message(report_now))
            await start_delayed_analysis(context, chat_id, loading_msg)
