...
def ai_conclusion(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if vol >= 0.85 and ent >= 0.85:
        return {
            ...
            "message": "Dữ liệu đang nhiễu cao, kết luận không ổn định.",
        }
    ...
    if streak >= 6:
        return {
            "status": "STREAK",
            "confidence": confidence,
            "message": f"Đang có chuỗi dài {report.get('last_label')} x{streak}.",
        }
    ...
    if cycle_score >= 70 and cycle_len:
        return {
            "status": "CYCLE",
            "confidence": confidence,
            "message": f"Phát hiện vòng lặp tương đối rõ, độ tin cậy {confidence}%.",
        }
    ...
    if gap >= 8:
        return {
            "status": "TRENDING",
            "confidence": confidence,
            "message": f"Xu hướng hiện tại đang nghiêng nhẹ, độ tin cậy {confidence}%.",
        }

    if gap <= 2 and total >= 20:
        return {
            "status": "BALANCED",
            "confidence": confidence,
            "message": "Hai phía đang khá cân bằng.",
        }
    ...
    if confidence < MIN_CONFIDENCE:
        return {
            "status": "UNCERTAIN",
            "confidence": confidence,
            "message": f"Tín hiệu còn yếu, chưa nên kết luận mạnh.",
        }

    return {
        "status": "NORMAL",
        "confidence": confidence,
        "message": f"Tín hiệu trung tính, độ tin cậy {confidence}%.",
    }


def render_result(report: Dict[str, Any], ai: Dict[str, Any]) -> str:
    hist = format_history(report.get("patterns", []))
    return hist
