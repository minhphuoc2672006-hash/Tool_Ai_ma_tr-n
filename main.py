...
def ai_anti_ghost(chat_id: int, report: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    elif cau_type == "CẦU LẶP LỆCH" and cycle_pred in (LOW_LABEL, HIGH_LABEL):
        scores[cycle_pred] *= 1.10
        scores[cycle_pred] += (cycle_pred_conf / 100.0) * 0.06
...
def final_decision(
    ...
    # Cân bằng chống dính một bên:
    # nếu lịch sử gần đây quá lệch hoặc hai bên gần như bằng nhau, đẩy nhẹ sang phía chưa bị “khóa”.
    ...
    if history_gap >= 6:
        if recent_low > recent_high:
            # Xỉu đang nhiều hơn -> ưu tiên Tài hơn một chút
            w_r -= 0.02
            w_g += 0.02
        else:
            w_r += 0.02
    ...
    # Nếu xác suất quá sát mà confidence thấp, hạ bớt để tránh chốt cứng.
    if diff < 0.03:
