...
def detect_cycle_repeat(labels: List[str], max_window: int = 12) -> Optional[Dict[str, Any]]:
    ...
    for size in range(2, upper + 1):
        ...
        if reps >= 2:
            ...
            cand = {"name": label, "detail": f"Nhịp run-length {'-'.join(map(str, motif))} lặp {reps} lần", "score": score, "hint": tail_runs[-1][0], "pattern_counts": motif, "signature": "-".join(map(str, motif))}
            if best is None or cand["score"] > best["score"]:
                best = cand
    return best
...
def predict_structure(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        hint = top.get("hint")
        name = top.get("name", "")
        strong_same = {'BỆT', 'BỆT SỚM', 'LẶP MẪU 1', 'LẶP KHỐI 1', 'XEN KẼ SỚM', 'XEN KẼ', 'XEN KẼ SÂU', "1-1", "2-2", 'SÓNG ĐỀU', 'CẦU PHỨC HỢP', 'CHU KỲ MỞ RỘNG'}
