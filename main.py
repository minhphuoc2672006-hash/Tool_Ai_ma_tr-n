...
def parse_input(text: str) -> Tuple[List[int], List[List[int]]]:
    """
    Returns:
      totals: list of total values (one per roll)
      dice_rolls: list of [d1, d2, d3]
    """
    text = (text or "").strip()
    if not text:
        return [], []
    ...
    # 1 token: accept either raw total or 3-digit dice face compact form like 356
    if len(tokens) == 1:
        tok = tokens[0]
    ...
    # Multiple tokens: if all are dice faces, group in threes
...
def build_trace_message(state: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"• Chờ hash: {state.get('hash_waiting', False)}\n"
...
def build_dice_summary(dice_rolls: List[List[int]]) -> Dict[str, Any]:
    if not dice_rolls:
        return {
            "total_rolls": 0,
    ...
    avg_faces = []
    for idx in range(3):
        face_sum = sum(face * cnt for face, cnt in die_counts[idx].items())
        cnt = sum(die_counts[idx].values())
        avg_faces.append(safe_div(face_sum, cnt))
...
def predict_dice(dice_rolls: List[List[int]], summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Simple, stable heuristic: focus on total and per-die drift
    if avg_total >= 11.6:
        label = HIGH_LABEL
    elif avg_total <= 10.4:
        label = LOW_LABEL
    ...
    if total_rolls >= 8:
        confidence += 6
    if hot_faces:
        confidence += min(10, int(hot_faces[0][1] / max(1, total_rolls)))
    ...
    for name, pred in predictions.items():
        ...
        if name == "chart":
            ...
            if abs(last_5_high - prev_5_high) >= 0.15:
                score *= 1.07
        if name == "dice":
            if abs(float(sum(adv.get("avg_faces", [0.0, 0.0, 0.0]))) / 3.0 - 3.5) >= 0.3:
                score *= 1.05
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str], dice_rolls_full: Optional[List[List[int]]] = None) -> Dict[str, Any]:
    ...
    dice_summary = build_dice_summary(dice_rolls)
...
def build_stats_message(report: Dict[str, Any], state: Dict[str, Any], adv: Dict[str, Any], dice_summary: Optional[Dict[str, Any]] = None) -> str:
    ...
    if dice_summary and int(dice_summary.get("total_rolls", 0)) > 0:
        ...
        dice_line = f"Rolls:{dice_summary.get('total_rolls', 0)} | Avg:{dice_summary.get('avg_total', 0.0):.2f} | D1/D2/D3:{av[0]:.2f}/{av[1]:.2f}/{av[2]:.2f}"
    return (
        "╔════════════════════════════╗\n"
        "║      ✅ BẢNG THỐNG KÊ      ║\n"
        "╠════════════════════════════╣\n"
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None, dice_rolls: Optional[List[List[int]]] = None) -> None:
    ...
    async with STATE_LOCK:
        ...
        if entries:
            ...
            if dice_rolls:
                # store only valid matched rolls in state (for dice chart)
