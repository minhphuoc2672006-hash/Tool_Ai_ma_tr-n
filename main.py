...
def cycle_consistency_score(labels: List[str], cycle_len: Optional[int]) -> float:
    """Đo độ đều của cầu lặp theo từng block chu kỳ.

    100 = các block gần như trùng nhau; thấp hơn khi chu kỳ bị méo, lệch nhịp hoặc gãy.
    """
    if not cycle_len or cycle_len < 2 or len(labels) < cycle_len * 3:
        return 0.0

    tail = labels[-min(len(labels), cycle_len * 6):]
    blocks = [tail[i:i + cycle_len] for i in range(0, len(tail), cycle_len) if len(tail[i:i + cycle_len]) == cycle_len]
    if len(blocks) < 2:
        return 0.0
    ...
    for block in blocks[:-1]:
        match = sum(1 for a, b in zip(block, ref) if a == b)
        exact_scores.append(match / cycle_len)

    if not exact_scores:
        return 0.0

    exact = sum(exact_scores) / len(exact_scores)
    # Phạt thêm nếu các block không đồng nhất với nhau.
    trans_changes = 0
    trans_total = 0
    for block in blocks:
        for i in range(1, len(block)):
            ...
            if block[i] != block[i - 1]:
                trans_changes += 1
    shape = (trans_changes / trans_total) if trans_total else 0.0

    score = (exact * 0.7 + min(shape, 1.0) * 0.3) * 100.0
...
def detect_cau_structure(report: Dict[str, Any]) -> Tuple[str, str]:
    ...
    cycle_consistency = float(report.get("cycle_consistency", 0.0))
    ...
    if cycle_score >= 70 and cycle_len and cycle_consistency < 60:
        return "CẦU LẶP LỆCH", f"Chu kỳ {cycle_len} (độ đều {cycle_consistency:.0f}%)"
...
def detect_mode(d: Dict[str, Any], report: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if cycle_score >= 70 and cycle_len and cycle_consistency < 60:
        return "CYCLE_WEAK", f"Vòng lặp lệch (size {cycle_len})"
    if gap >= 20:
...
def analyze_sequence(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cycle_consistency = cycle_consistency_score(labels, cycle_len)
...
def ai_new_rhythm(chat_id: int, report: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    if cycle_score >= 60 and cycle_len:
        ...
        if cycle_pred in (LOW_LABEL, HIGH_LABEL):
            scores[cycle_pred] += conf / 100.0 * 0.12
    if vol >= 0.65:
...
def final_decision(
    ...
    # Chống dính một phía: nếu 20 lượt gần nhất lệch mạnh thì nới trọng số phía còn lại.
    ...
    # Cầu lặp yếu thì không được ép quá tay.
    if cycle_score >= 70 and cycle_len and cycle_consistency < 60:
        w_r *= 0.93
        w_g *= 0.93
    ...
    # Chống default về Xỉu khi hai bên sát nhau
    if diff < 0.025:
        ...
        if recent_low_all == recent_high_all:
            last_label = report.get("last_label")
            # Nếu đang bệt Xỉu quá lâu thì nhả sang Tài, còn lại thì giữ cân bằng theo xu hướng gần nhất.
        ...
        # Dùng ngưỡng chênh lệch để chọn, tránh "kẹt" theo dict order.
        if probs[HIGH_LABEL] > probs[LOW_LABEL]:
            best_label = HIGH_LABEL
        elif probs[LOW_LABEL] > probs[HIGH_LABEL]:
            best_label = LOW_LABEL
        else:
            last_label = report.get("last_label")
            best_label = HIGH_LABEL if last_label == LOW_LABEL else LOW_LABEL if last_label == HIGH_LABEL else HIGH_LABEL
    ...
    # Hạ trần khi tín hiệu chưa thật sạch hoặc cầu lặp không đều.
    if diff < 0.03 or recheck_status == "RECHECK_WATCH":
        confidence = min(confidence, 68)
    if cycle_score >= 70 and cycle_len and cycle_consistency < 60:
        confidence = min(confidence, 66)
    if volatility >= 0.45 and entropy >= 0.80 and diff < 0.08:
